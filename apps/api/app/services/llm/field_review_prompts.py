...
SAFETY_RULES = (
    ...
    "- The pack's known_gaps state EXACTLY which identity fields are missing "
    "for EVERY company and which for only SOME. Use those statements verbatim "
    "rather than merging the per-company lists yourself; a field listed as "
    "missing for SOME must never be described as missing for all.\n"
