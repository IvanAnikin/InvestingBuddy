...
class RunCandidateAnalysisResponse(BaseModel):
    """Async full-analysis JOB state for one discovery candidate.

    Product readiness — the "Run Full Analysis" POST used to run the whole
    pipeline inline and regularly blew the ~230s Azure gateway ceiling, so the
    admin saw HTTP 504 while the backend quietly finished and persisted a good
    final report. The POST now returns 202 with this envelope in ``pending``
    and the UI polls GET ``/candidates/{id}/analysis-job`` for the rest.

    ``status`` is the JOB lifecycle state (pending | running | interrupted |
    completed | completed_with_warnings | failed) — never an investment action.

    ``interrupted`` (private-use readiness PR-F) is DERIVED at read time, not
    stored: a background job runs in-process, so an app restart mid-run leaves
    the stored envelope on ``running`` forever. The state was always
    recoverable, but nothing said so. ``recoverable`` tells the caller that
    ...
    ticker: str
    status: str
    # Job lifecycle timestamps (ISO-8601 UTC) — None until the phase is reached.
    started_at: str | None = None
    completed_at: str | None = None
    # The underlying workflow's own terminal status, once the job finishes.
    workflow_status: str | None = None
    # Safe, non-secret failure reason for a ``failed`` job.
    error: str | None = None
    # Private-use readiness PR-F — set for an ``interrupted`` job: its worker is
    # gone (an app restart mid-run) and re-running is safe.
    ...
    def from_job_envelope(
        ...
        *,
        ...
        """Adapt a stored analysis-job envelope into the API response."""
        ...
        report_raw = envelope.get("report")
        report = (
            ReportLinkSummary.model_validate(report_raw)
