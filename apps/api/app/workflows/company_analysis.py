...
from app.services import agent_run_service, company_service, report_service
...
def build_company_analysis_graph(db: AsyncSession):
    ...
    async def node_save_report(state: CompanyAnalysisState) -> dict:
        ...
        summary = (
            f"Company analysis draft for {state.get('company_name', ticker)}. "
        ...
        content_md = (
            f"# {state.get('company_name', ticker)} — Draft Analysis\n\n"
            f"**Rating:** {analysis.get('rating', 'WATCH')}  \n"
            f"**Confidence:** {analysis.get('confidence_score', 0):.0%}  \n"
            ...
            + "\n\n---\n\n*Placeholder output. Human review required.*\n"
        )

        report = await report_service.create_draft_report(
            db,
            ReportCreate(
                title=f"{state.get('company_name', ticker)} — Draft Analysis",
                slug=slug,
        ...
        await agent_run_service.complete_agent_step(
            db,
            step,
            output_data={"report_id": str(report.id), "slug": report.slug},
        )

        return {"draft_report_id": str(report.id)}
