...
# ── V3.19.8: a cash-FLOW line is not a cash balance (live Kering, URD 2025 p.54) ── #
...
def test_a_cash_flow_row_is_never_net_cash(row):
    assert _match_label(row) != "net_cash"


def test_kering_operating_cash_row_is_operating_cash_flow():
    assert _match_label("Net cash received from operating activities") == "operating_cash_flow"


def test_kering_prose_net_cash_received_is_not_a_balance():
    got = _by_field("Net cash received from operating activities 3,100 4,709 -34%")
    assert not got.get("net_cash")


def test_a_real_net_cash_balance_still_reads():
    assert _match_label("Net cash position") == "net_cash"
    assert _by_field("Net cash position of €1,200 million at year end.").get("net_cash") == [
        1200.0
    ]
