...
# ===========================================================================
# The real client must APPLY the per-call budget (not just accept it)
# ---------------------------------------------------------------------------
# Live regression (2026-08-23): raising the chair's computed budget from 6,040
# to 8,400 changed NOTHING — the same two agents truncated identically. Cause:
# ``AzureOpenAILLMClient._complete_raw`` accepted ``max_tokens`` and never
# forwarded it, so every real call used the CONSTRUCTION-time default (1200).
# Every count-aware output budget in the codebase — this slice's and the
# discovery council's — was inert against the real provider.
#
# The fake client honours ``max_tokens`` faithfully, which is exactly why the
# earlier tests passed while production truncated. These tests assert the real
# invocation path, using a stub model object (no credentials, no network).
# ===========================================================================
...
async def test_ainvoke_forwards_the_per_call_max_tokens() -> None:
    from app.services.llm.azure_openai_client import _ainvoke_chat

    stub = _StubChatModel()
    await _ainvoke_chat(stub, "sys", "user", 30, max_tokens=8400)
    assert stub.calls[-1]["max_tokens"] == 8400, (
        "the per-call output budget must reach the provider, or every "
        "count-aware budget in the codebase is silently inert"
    )


async def test_ainvoke_omits_max_tokens_when_unset() -> None:
    """No per-call value => fall back to the model's construction-time default."""
    from app.services.llm.azure_openai_client import _ainvoke_chat

    stub = _StubChatModel()
    await _ainvoke_chat(stub, "sys", "user", 30)
    assert "max_tokens" not in stub.calls[-1]
    stub2 = _StubChatModel()
    await _ainvoke_chat(stub2, "sys", "user", 30, max_tokens=0)
    assert "max_tokens" not in stub2.calls[-1]
...
async def test_real_client_complete_raw_passes_its_budget_through() -> None:
    ...
    assert stub.calls[-1]["max_tokens"] == 6040
