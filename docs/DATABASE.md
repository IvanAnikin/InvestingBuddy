...
| `excerpts_json` | JSONB nullable | a bounded JSON ARRAY of excerpt dicts (never full text / raw table grid); each carries its own `extraction_method`. **Phase 32A Slice 5B.2**: an OCR-recovered excerpt (`extraction_method: "ocr"`) persists through this SAME unchanged path once OCR has promoted the document to `extracted` — no schema change was needed for excerpts. **Pre-existing, still-open gap (unchanged by this slice, native OR OCR):** `PrimaryDocumentExtraction.tables` is not persisted anywhere yet — a validated fact's own `page_number`/`table_location` (on `extracted_facts`, below) is the only durable table provenance today. Persisting the bounded table grid itself (and sanitized OCR provider/model/version metadata) remains a follow-up, tracked in ADR-016. |
| `pipeline_version` | int nullable | **Migration 016 (Phase 32A corrective, Problem B).** The extraction/parsing/validation pipeline version (`app.services.sources.extraction_pipeline_version.CURRENT_EXTRACTION_PIPELINE_VERSION`) active when this row's `excerpts_json` + `extracted_facts` children were produced. NULL = written before this column existed — treated as legacy/stale. `load_reusable_documents` reuses `excerpts_json` (content layer) regardless, but only trusts the persisted `extracted_facts` rows as-is when this matches the CURRENT version; on a mismatch it RE-DERIVES facts from the reused excerpts under current-code semantics (no re-fetch) instead of trusting a stale row unchanged. |
