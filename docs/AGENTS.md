...
| catalyst_discovery_agent | CatalystDiscoveryAgent | catalyst_discovery | **Phase 24:** runs for `free_real`/`eodhd_free_real` only (mock unchanged). Calls `discover_catalysts` (SEC recent filings T2 + company press releases T1 + optional news T5), classifies each event (category/direction/strength/evidence, always T6 model label), and via `run_catalyst_agent` produces the News & Catalyst Discovery report sections + council context (bull/bear/risk/committee/source-quality). Non-blocking; never emits recommendations, price targets, fair values, or upside/downside |
