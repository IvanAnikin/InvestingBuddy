...
`T2_regulator_or_gov`), all promoted from planned.
...
**Phase 29D.2 — patent event-trigger reference evidence (PR open / pre-staging** at
`42f80a7`**).** Extends the 29D.1 event-trigger layer to **patents** with **zero new
...
fetch; no API key) — reusing `SOURCE_EVENT_ENABLED`.
