...
```json
{
  ...
  "summary": { "enabled": 6, "configured": 3, "scaffolded": 6, "planned": 19, "disabled": 0, "total": 31 },
...
`stooq`, `gdelt`. **Scaffolded** filing/regulator connectors (Phase 29B — class
exists, returns honest gaps, no live fetch yet): SEDAR+, ASX, UK FCA NSM,
Euronext, Deutsche Börse, Nordic. Planned placeholders (disabled, surfaced as
...
live fetch (SEDAR+, ASX, UK FCA NSM, Euronext, Deutsche Börse, Nordic).
...
> behaviour; it is **not yet merged / deployed / staging-validated**. Treat the
> API surface as stable-on-branch but unverified in staging until the merge SHA +
> deployed SHA + validation are on file.
...
primary-source packs stay under the Azure OpenAI TPM ceiling.
