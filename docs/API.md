...
```json
{
  ...
  "summary": { "enabled": 28, "configured": 3, "scaffolded": 2, "planned": 5, "disabled": 0, "total": 35 },
...
(`usaspending.gov`), both promoted from planned.
...
surfaced as source gaps): OpenBB, Google Patents, USPTO, EPO Espacenet,
local-language business press.
...
**Phase 29D.1 — procurement / tender event-trigger reference evidence (PR open /
pre-staging** at `63eafa3`**).** A NEW event-trigger evidence category, parallel to
...
— OFF by default behind `SOURCE_EVENT_ENABLED`.
