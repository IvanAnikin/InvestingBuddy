...
```json
{
  ...
  "summary": { "enabled": 6, "planned": 25, "disabled": 0, "total": 31 },
...
Enabled (migrated) sources: `sec_edgar`, `company_ir`, `gleif`, `eodhd`,
`stooq`, `gdelt`. Planned placeholders (disabled, surfaced as source gaps):
SEDAR+, ASX, UK FCA NSM, Euronext, Deutsche Börse, Nordic, USGS, IEA, IRENA,
EIA, ENTSO-E, World Bank Pink Sheet, FRED, IMF, Eurostat, USTR/TARIC,
USAspending, EU TED, UN Comtrade, national stats/central banks, OpenBB, Google
Patents, USPTO, EPO Espacenet, local-language business press.
...
`status` is one of `enabled` / `configured` / `not_configured` / `planned` /
`disabled` / `not_implemented` / `error`.
