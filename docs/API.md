...
```json
{
  ...
  "summary": { "enabled": 34, "configured": 3, "scaffolded": 2, "planned": 2, "disabled": 0, "total": 38 },
...
surfaced as source gaps): OpenBB, local-language business press.
...
> **Status: PR open — pre-staging.** This section documents merged-branch
> behaviour; it is **not yet merged / deployed / staging-validated**. **No new
> endpoint, no DB migration.** The only changes are additive **persisted-metadata
> / report fields** on existing flows, gated behind four new OFF-by-default flags.
