...
```json
{
  ...
  "summary": { "enabled": 21, "configured": 3, "scaffolded": 2, "planned": 9, "disabled": 0, "total": 32 },
...
multilingual and major issuers publish English). **Phase 29C** adds ten
**enabled reference-only macro / commodity-energy** sources (each emits a bounded
`macro_report` SOURCE REFERENCE + an honest `data_not_sourced` gap; **no live
figures**; OFF-by-default behind `SOURCE_MACRO_ENABLED`) — **29C.1 macro**
(`T2_regulator_or_gov`): `fred`, `imf`, `eurostat`, `national_stats_central_banks`
(`macro_statistics`) and `world_bank_pink_sheet` (`commodity`); **29C.2 commodity
+ energy** (`commodity`): `usgs`, `iea`, `irena`, `entsoe`
(`T3_industry_specialist`) and `eia` (`T2_regulator_or_gov`, no API key).
**Scaffolded** filing/regulator connectors (Phase 29B — class exists, returns
honest gaps, no live fetch yet): SEDAR+, ASX. Planned placeholders (disabled,
surfaced as source gaps): USTR/TARIC, USAspending, EU TED, UN Comtrade, OpenBB,
Google Patents, USPTO, EPO Espacenet, local-language business press.
...
**Phase 29C.1 + 29C.2 — macro / commodity-energy reference evidence** (29C.1 macro
**merged + staging-validated at `a8ac580`**; 29C.2 commodity + energy **PR open /
pre-staging** at `8c62297`). Ten official public sources are promoted from
**planned** to **enabled reference-only** connectors (a single generic
`MacroReferenceConnector`, one instance per `ALL_MACRO_SOURCES` spec) — **29C.1
macro** (`T2_regulator_or_gov`): `fred` (`fred.stlouisfed.org`), `imf`,
`eurostat`, `world_bank_pink_sheet`, `national_stats_central_banks`; **29C.2
commodity + energy** (`commodity`): `usgs` (`usgs.gov`, T3; copper / lithium /
rare-earths / critical minerals / uranium), `eia` (`eia.gov`, T2, no API key;
uranium / nuclear / oil / gas / electricity), `iea` (`iea.org`, T3; energy /
...
/ transmission). Each `fetch_macro_context` emits, for a relevant theme/region,
...
tonnage, price, capacity, production, release date, or forecast is ever emitted,
**network-free**, **no API key**. This layer is **OFF by default** behind `SOURCE_MACRO_ENABLED`
...
endpoint.** **Deliberate deferral:** live macro / commodity / energy FIGURE fetch
is a documented 29C follow-up (keyless official-data APIs). See the **Phase 29C.2**
...
`world_bank_pink_sheet`) and **Phase 29C.2** promoted five commodity / energy
sources (`usgs`, `iea`, `irena`, `entsoe` — `T3_industry_specialist`; `eia` —
`T2_regulator_or_gov`, no API key) to **enabled** reference-only `macro_statistics`
/ `commodity` connectors that each emit a bounded macro SOURCE REFERENCE + an
honest `data_not_sourced` gap (live figures not fetched at report time; no API
key).
