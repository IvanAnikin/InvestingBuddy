...
```json
{
  ...
  "summary": { "enabled": 8, "configured": 3, "scaffolded": 4, "planned": 19, "disabled": 0, "total": 31 },
...
`stooq`, `gdelt`, plus two **enabled regulated-disclosure regulator reference**
...
(UK FCA NSM/RNS) and **Phase 29B.4B** `euronext_regulated_info` (Euronext
Paris/Amsterdam regulated-information + AMF/AFM; French-jurisdiction issuers also
carry a `requires_translation` gap). **Scaffolded** filing/regulator connectors
(Phase 29B — class exists, returns honest gaps, no live fetch yet): SEDAR+, ASX,
...
live fetch (SEDAR+, ASX, Deutsche Börse, Nordic). **Phase 29B.4A** promoted
`uk_fca_nsm` and **Phase 29B.4B** promoted `euronext_regulated_info` out of this
set to **enabled `regulator`** reference connectors (`T2_regulator_or_gov`) that
emit a bounded venue reference + an honest content gap (T1 filing content not
