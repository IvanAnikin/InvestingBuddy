...
```json
{
  ...
  "summary": { "enabled": 16, "configured": 3, "scaffolded": 2, "planned": 14, "disabled": 0, "total": 32 },
...
multilingual and major issuers publish English). **Phase 29C.1** adds five
**enabled reference-only macro** sources (`T2_regulator_or_gov`; each emits a
bounded `macro_report` SOURCE REFERENCE + an honest `data_not_sourced` gap; **no
live figures**; OFF-by-default behind `SOURCE_MACRO_ENABLED`): `fred`, `imf`,
`eurostat`, `national_stats_central_banks` (`macro_statistics`) and
`world_bank_pink_sheet` (`commodity`). **Scaffolded** filing/regulator
connectors (Phase 29B — class exists, returns honest gaps, no live fetch yet):
SEDAR+, ASX. Planned placeholders (disabled, surfaced as
source gaps): USGS, IEA, IRENA, EIA, ENTSO-E, USTR/TARIC, USAspending, EU TED,
UN Comtrade, OpenBB, Google Patents, USPTO, EPO Espacenet, local-language
business press.
...
**Phase 29C.1 — macro reference evidence (PR open / pre-staging).** Five official
public macro sources — `fred` (`fred.stlouisfed.org`), `imf`, `eurostat`,
`world_bank_pink_sheet`, `national_stats_central_banks` — are promoted from
**planned** to **enabled reference-only macro** connectors (a generic
`MacroReferenceConnector`). Each `fetch_macro_context` emits, for a relevant
theme/region, ONE bounded **T2 `macro_report` SOURCE REFERENCE** to a fixed
public token-free landing page describing *which indicators the dataset covers*,
plus an honest `data_not_sourced` gap. It is **reference-only**: no indicator
value, index level, release date, or forecast is ever emitted, **network-free**,
**no API key**. This layer is **OFF by default** behind `SOURCE_MACRO_ENABLED`
...
endpoint.** **Deliberate deferral:** live macro-FIGURE fetch is a documented 29C
follow-up (keyless official-data APIs). See the **Phase 29C.1** note in the Status
section above.
...
`world_bank_pink_sheet`) to **enabled** reference-only `macro_statistics` /
`commodity` connectors (`T2_regulator_or_gov`) that emit a bounded macro SOURCE
REFERENCE + an honest `data_not_sourced` gap (live figures not fetched at report
time; no API key).
