...
```json
{
  ...
  "summary": { "enabled": 7, "configured": 3, "scaffolded": 5, "planned": 19, "disabled": 0, "total": 31 },
...
`stooq`, `gdelt`, plus **Phase 29B.4A** `uk_fca_nsm` — an **enabled UK
regulated-disclosure regulator reference** connector (`T2_regulator_or_gov`;
emits a bounded FCA NSM/RNS venue reference + an honest content gap; no live
content fetch yet). **Scaffolded** filing/regulator connectors (Phase 29B —
class exists, returns honest gaps, no live fetch yet): SEDAR+, ASX, Euronext,
Deutsche Börse, Nordic. Planned placeholders (disabled, surfaced as
source gaps): USGS, IEA, IRENA, EIA, ENTSO-E, World Bank Pink Sheet, FRED, IMF,
Eurostat, USTR/TARIC, USAspending, EU TED, UN Comtrade, national stats/central
banks, OpenBB, Google Patents, USPTO, EPO Espacenet, local-language business
press.
...
live fetch (SEDAR+, ASX, Euronext, Deutsche Börse, Nordic). **Phase 29B.4A**
promoted `uk_fca_nsm` out of this set to an **enabled `regulator`** reference
connector (`T2_regulator_or_gov`) that emits a bounded FCA NSM/RNS venue
reference + an honest content gap (T1 filing content not fetched at report time).
