...
## Status: Phase 19.3.1 — SEC freshness + review-consistency hotfix on top of Phase 19.3. The SEC normalizer (`sec_fundamentals_normalizer`) selects the latest annual filing across all us-gaap alias concepts (filed-date tiebreak, full-year preference, stale-year warning) so no outdated "latest annual FY2018" surfaces; `investment_committee_chair` emits a canonical `human_review_required` consistent with report/page metadata; `BearCaseAgent`/`RiskAgent` acknowledge partial SEC fundamentals. Underlying 19.3: SEC XBRL fundamentals normalized into income-statement / cash-flow / balance-sheet metrics + derived ratios, injected into the `free_real` snapshot; `FinancialDataAgent` narrates them, `ValuationGuardAgent` reaches `partial` while still blocking all valuation conclusions (EBITDA / market cap / EV / shares never fabricated). Builds on Phase 19.2.1 (provider observability), Phase 19.1 Free Real Data Stack, Phase 22.1 Admin Backtesting UI. 920 backend tests passing (12 skipped, integration-only).
