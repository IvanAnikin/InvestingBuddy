...
| `NullNewsProvider` / `EnvConfiguredNewsProvider` | `integrations/providers/free_news_provider.py` | T5 | ✅ Live (Phase 24) | Optional news abstraction; **null by default** (no key, no CI call); env-gated generic JSON provider via `NEWS_PROVIDER_NAME`/`NEWS_API_KEY`/`NEWS_API_BASE_URL` |
