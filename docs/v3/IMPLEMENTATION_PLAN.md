...
| **V3.9** | Monitoring | `IMPLEMENTED` — 9.1 merged. |
...
| 4.10 | [`feature/v3-4-10-issuer-site-traversal`](slices/V3.4-10-issuer-site-traversal.md) | Bounded same-domain walk on the existing guarded fetcher. Four finite bounds and `stopped_by` always populated; `robots.txt` obeyed by **not requesting**; a JS-gated page recorded as partially inaccessible. No paid crawler. | `IMPLEMENTED` |
...
| 8.2 | [`feature/v3-8-2-research-delta`](slices/V3.8-2-research-delta.md) | What changed, and — harder — what did **not**: a rephrasing, a new period, a new scope and silence are each refused as changes. The prior finding is never modified. `IMPLEMENTED` (037). |
| 9.1 | [`feature/v3-9-1-monitoring`](slices/V3.9-1-monitoring.md) | Watchlists, change detection and signals — **feature-gated with nothing scheduling it** ([#15](OPEN_DECISIONS.md#15-monitoring-cadence) is user-owned). An issuer with no prior research produces no signals: that is what stops it being a feed. `IMPLEMENTED` (038). |
...
V3_ISSUER_TRAVERSAL_ENABLED
V3_MONITORING_ENABLED
```
