...
## Current State: Phase 29B.4C Swiss / Nordic / Germany Regulated-Disclosure Connectors 🔜 NEXT — not started (no branch yet). The final planned subphase of the Phase 29B.4 umbrella (EU/UK regulated-disclosure connectors). Continues the **promote-scaffold→connector pattern proven in 29B.4A (UK) and 29B.4B (Euronext)**: promote the `deutsche_boerse` and `nordic_disclosures` scaffolds to dedicated connectors — `SAP.DE` SAP (Deutsche Börse / Xetra DE → Bundesanzeiger / BaFin regulated-information venue) and `PNDORA.CO` Pandora (Nasdaq Copenhagen / Nordic DK) — each emitting ONE bounded **T2 regulated-information venue *reference*** + honest `primary_filing_unavailable` content gap, **network-free at report time**, with German- and Danish-jurisdiction issuers **flagged `requires_translation`** (`GapType.translation_required`; pending Phase 30 — no local-language ingestion). **Honest gap — Swiss / SIX:** the intended Swiss targets `UHR.SW` Swatch and `CFR.SW` Richemont trade on SIX Swiss Exchange, for which there is currently **NO scaffold** in the registry (scaffold set is SEDAR+ / ASX / Deutsche Börse / Nordic) — so covering them requires a **new SIX/Swiss connector built from scratch (not a scaffold promotion)** or an honest deferral; this is called out up front so 29B.4C does not silently promise a promotion that has no scaffold to promote. **Deferral carried forward (from 29B.4A/4B):** the live regulator-venue CONTENT fetch stays deferred (honest T2 venue reference + explicit content gap is the chosen posture). Safety posture unchanged: evidence-first, citation-bound, no recommendation/rating/BUY-SELL-HOLD-WATCH/price-target/fair-value/upside; `human_review_required=true` / `publication_ready=false`; `/admin/*` OAuth-gated; human approval before publication. **Next: create branch `feature/phase-29b4c-swiss-nordic-de-disclosures` and scope 29B.4C; then 29C (macro/commodity/policy), 30 (translation/local-language + PDF table extraction / OCR).**
