...
> production remains untouched. The phase narrative below predates that work
> and is retained for history — read `docs/DECISIONS.md` (ADR-018 to ADR-022)
> and the CLOSED/OPEN tables in `docs/GAP_ANALYSIS.md` for current state.
>
> **Next campaign:** Phase D — European source enrichment (Nasdaq Nordic
> first), then E (research quality) and F (observability/test hardening).


## Current State: Phase 31 Source-Aware Internal Research Memo Builder 🟡 IN PROGRESS — PR open / pre-staging (the **FINAL campaign phase**). **NOT merged / deployed / staging-validated** (branch `feature/phase-31-research-memo-builder` @ `618344d`; PR about to open). Full-stack, **7 files incl. tests**, **NO migration** (DB head `011`), **no new host/endpoint/secret**; ONE new OFF-by-default flag `SOURCE_RESEARCH_MEMO_ENABLED` (`source_research_memo_enabled=false`). With the Phase 30 umbrella now ✅ COMPLETE (30A translation foundation `fa3632a` + 30B local-language evidence sources `e1d2d8d`, both merged + deployed + staging-validated), this is the **last phase** — a source-aware **INTERNAL admin research MEMO builder**, OFF by default. A new backend `_build_research_memo(report_content, council_result)` (`app/services/final_report_generator.py`) is a **DETERMINISTIC synthesis** of the already-assembled `report_content` sections + `CouncilResult` metadata + known gaps — **no external call, no LLM, no ORM, no recompute** — emitting `report_content["research_memo"]` (attached BEFORE the safety gate) with: **header** (internal-only / not-advice) · **company identity** · **why surfaced** · **what's sourced** · **what's missing (PROMINENT)** · **primary evidence** (primary_documents + primary_facts, cited with token-stripped `source_urls`) · **catalyst / event evidence** · **financial facts** (T5 EODHD + T1 `*_primary_filing` datapoints; **no derived valuation**) · **business / risk** · **council disagreement / red-team** (the `red_team` agent's dissent + `unsupported_claims` + committee label) · **research next steps** · **human-review checklist** (references the existing checklist, no recompute) · **source appendix** · a **`disallowed_outputs` notice** · note + disclaimer + `human_review_required=True`. **Citation-bound** — each claim ties to an existing provenance / source / citation. The forbidden BUY/SELL/HOLD/WATCH + price-target / fair-value / upside / downside literals appear ONLY in the scanner-exempt `disallowed_outputs` field; **every other memo field is safety-clean** (memo attached before the safety gate). **Honest degradation** on thin evidence (`what_is_missing` prominent, `provenance=missing_data`, honest-empty sections, never fabricates). NOT in `_REQUIRED_SECTIONS` (so `schema_valid` is unaffected); `publication_ready` False; `human_review_required` True; **dark-by-default byte-identical when off** — and the memo happy-path IS staging-demonstrable (flip the flag → a real report includes the memo, since it derives from existing report data). Frontend: `research_memo` added to `SECTION_ORDER` + an "Internal Research Memo" label; a bespoke `ResearchMemoSection` renders the memo's sub-blocks readably in the **Readable** tab (Raw JSON stays the hidden-by-default developer tab; legacy reports without the memo render unchanged); `disallowed_outputs` renders as a plain NOTICE (no rating / BUY-SELL UI). Same discipline: evidence-first, citation-bound, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside outside the exempt notice; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated; human approval before any publication. Verified GREEN (pre-staging): backend **2289 pass / 12 skip / 0 fail**, ruff clean, mypy `71` baseline (no new); web typecheck / lint / build pass; e2e **196/196**; security scan PASS. **Phase 31 is PR-open — STOP at the merge gate; on merge + staging validation it CLOSES Phase 31, and as the FINAL phase completes the entire product roadmap through Phase 31.** **Next command:** run `ib-pr-review-agent` then `gh pr create` for Phase 31; STOP at the merge gate.

### Previously: Phase 30B Local-Language Business-Press Evidence Sources ✅ COMPLETE — merged + deployed + staging-validated (VALIDATED, clean — no environmental note) (PR #68 squash-merged to `main` at `e1d2d8d`; API `commit_sha=e1d2d8d` on 3 stable polls; web unchanged — backend-only, no web change; DB head `011`, **no migration, no new host/endpoint/secret, NO new flag** — reuses the OFF-by-default `SOURCE_CONNECTOR_ENABLED` for collection + the 30A `SOURCE_TRANSLATION_ENABLED` for translation). **The second Phase 30 subphase — its closure COMPLETES the entire Phase 30 umbrella (30A translation foundation + 30B local-language sources).** With **Phase 30A closed + staging-validated** (the language-detection + translation foundation on `main` at `fa3632a`, OFF by default), 30B adds the **local-language evidence SOURCES** the 30A layer was built to translate. A new **`LocalLanguagePressConnector`** (`sources/connectors/local_language_press.py`) + a `LOCAL_LANGUAGE_PRESS_SOURCES` allowlist of reputable local-language business-press venues (**Les Échos** FR / **Handelsblatt** DE / **Milano Finanza** IT / **Børsen** DK — fixed public HTTPS landing pages, no API key) emits, for a **verified non-US FR/DE/IT/DA issuer**, ONE bounded **T4_quality_media `news_article` SOURCE REFERENCE** (provider `news`) carrying a **GENUINE short local-language descriptive excerpt** (what the venue covers + that the full article content is NOT fetched — **never a fabricated article / headline / quote / figure / date**), marked `requires_translation` + `original_language`, `low` confidence / `metadata_only`, with an honest `translation_required` + content-not-fetched gap + `needs_human_review` (it deliberately **lowers** source quality — a WEAK research-priority signal, never a recommendation / catalyst / materiality / valuation); a non-eligible / US issuer → honest `source_not_eligible` gap, never a reference; **network-free**. `company_evidence.py` gains a dedicated `LOCAL_LANGUAGE_REFERENCE_IDS` collection block that runs the connector for verified non-US FR/DE/IT/DA issuers (regulator mapping untouched); `registry.py` promotes `local_language_business_press` PLANNED→**enabled** (`news` / `T4_quality_media`, `PHASE_30B` label) → **35 enabled / 2 scaffolded / 1 planned / 38 total** (only `openbb` remains planned; SEDAR+/ASX scaffolds). It **consumes the 30A translation layer** (with `SOURCE_TRANSLATION_ENABLED` on, the non-English excerpt gets a bounded per-excerpt, machine-assisted / NOT-official, human-review-required translation with the original text + `source_url` preserved as the citation of record) — but the local-language REFERENCE itself is emitted under the already-ON `SOURCE_CONNECTOR_ENABLED`, **independent of** the translation flag. Keep **source gaps honest** (honest `data_not_sourced` / `translation_required` gaps, never fabricated headlines / figures / filings); **allowlisted only — NO broad / arbitrary web search**. Same discipline: evidence-first, citation-bound, source-registry + `EvidenceItem` tiers + explicit `SourceGap`s, OFF-by-default flags, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated; dark-by-default byte-identical when off. Backend **2278 pass / 12 skip / 0 fail** (+14 over 30A; new `test_phase30b_local_language_sources.py`; adjacent count tests updated to **35 enabled / 1 planned**, no ripple), ruff clean, mypy `71` baseline (no new), security scan PASS (ib-security-agent — reference-only, network-free, allowlisted-only, no API key / new host / secret, genuine-not-fabricated local-language excerpt, deliberately quality-lowering T4/low/needs-review, no reco/valuation; pre-PR review APPROVED 10/10); 15 files (incl. tests). **Staging validation (2026-07-29) — VALIDATED (clean, no environmental note):** merged `e1d2d8d`, API `commit_sha=e1d2d8d` (3 stable polls), web unchanged (backend-only), no migration (head `011`), AUTH_TEST_MODE absent, publication admin-gated. Unlike 30A, 30B was **directly demonstrable on staging in the default flag state** — the local-language SOURCE REFERENCES surface in evidence-preview under the already-ON `SOURCE_CONNECTOR_ENABLED`; only the *machine translation* of the excerpt needs the 30A flag (kept OFF). **B — registry/health:** `local_language_business_press` **enabled** (`news` / **T4**), summary **35 enabled / 2 scaffolded / 1 planned (only `openbb`) / 38 total**, honest reference-only note, secret-free. **C — evidence-preview (AC1/AC2 directly demonstrated, `SOURCE_TRANSLATION_ENABLED` OFF):** authed `POST /sources/evidence-preview` on four verified non-US issuers each returns the local-language reference with `requires_translation=true`, the correct `original_language`, a **genuinely NON-ENGLISH excerpt**, and an honest `translation_required` gap — `MC.PA` (LVMH FR) → **French** `lesechos.fr`, `SAP.DE` (DE) → **German** `handelsblatt.com`, `MONC.MI` (Moncler IT) → **Italian** `milanofinanza.it`, `PNDORA.CO` (Pandora DK) → **Danish** `borsen.dk`; the pre-existing `company_ir` / regulator references are co-present (no regression). **D — US guardrail:** `AAPL` (US) → **no** local-language item (honest `source_not_eligible`). **E — safety:** genuine local-language descriptions (no fabricated article / headline / quote / figure / date), tier **T4 / low / needs-review**, no recommendation / valuation language, admin-gated. **F — hygiene:** AUTH_TEST_MODE absent, logs clean, flags correct. **Decision:** **`SOURCE_TRANSLATION_ENABLED` KEPT OFF on staging** — conservative default; the local-language references still appear in evidence-preview with their translation state flagged (C), so the machine-assisted translation path stays dormant until a human flip; the ON-state `translated_evidence` render is fixture-proven + optional. **Deliberate deferral (carried):** live local-language CONTENT fetch is DEFERRED (reference-only) — the connector emits a venue reference + genuine descriptive excerpt, never the full article body; a live local-language content fetch (allowlisted, bounded, reviewed) is a documented follow-up (mirrors the 29B.4 / 29C / 29D / 30A reference-only deferrals). Final staging flags (6 ON, `SOURCE_TRANSLATION_ENABLED` OFF): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true · `SOURCE_MACRO_ENABLED`=true · `SOURCE_EVENT_ENABLED`=true · **`SOURCE_TRANSLATION_ENABLED`=false (kept OFF)** · `TRANSLATION_PROVIDER`=`fake` (default). **Carry-forward limitations:** (1) reference-only — each item is a venue SOURCE REFERENCE with a genuine descriptive excerpt, not article content (live content fetch deferred); (2) the translation happy-path (ON-state `translated_evidence` render) stays gated OFF — fixture-proven + optional, needs a human flip, though the non-English references themselves are now staging-demonstrable; (3) WEAK signal, deliberately quality-lowering (T4 / low / `metadata_only` / `needs_human_review`) — never a recommendation / catalyst / materiality / valuation; (4) allowlisted-only (four curated FR/DE/IT/DA venues) — extending the allowlist is a bounded reviewed follow-up; (5) Azure OpenAI gpt-4.1-mini TPM quota remains a standing staging environmental limiter (not triggered by this reference-only, network-free subphase). Closure report: `docs/development/closures/phase-30b.md`. **This closure COMPLETES the entire Phase 30 umbrella** (30A `fa3632a` + 30B `e1d2d8d`) — translation foundation + local-language evidence sources; PDF table extraction + OCR are deferred later-area work, not blocking Phase 31. **Next: Phase 31 — source-aware internal research memo builder (the FINAL phase)** — an internal admin research memo assembled from existing evidence packs / source gaps / council outputs / primary facts / red-team critiques; citation-bound; degrades honestly when evidence is thin; **no public publishing, no recommendation labels, no valuation, no price targets**; memo structure per the campaign spec (header internal-only-not-advice · company identity · why surfaced · what's sourced · what's missing · primary evidence · catalyst / event evidence · financial facts · business / risk · council disagreement / red-team · research next steps · human-review checklist · source appendix · disallowed-outputs notice).

### Previously: Phase 30A Language Detection + Translation Foundation ✅ COMPLETE — merged + deployed + staging-validated (OFF-state) (PR #67 squash-merged to `main` at `fa3632a`; API `commit_sha=fa3632a` on 3 stable polls; web unchanged — backend-only, no web change; DB head `011`, **no migration, no new host/endpoint/secret**; four new OFF-by-default flags `SOURCE_TRANSLATION_ENABLED` / `SOURCE_TRANSLATION_MAX_CHARS`(400) / `SOURCE_TRANSLATION_MAX_EXCERPTS`(3) / `TRANSLATION_PROVIDER`(`fake`)). A **NEW capability area** (not a Phase 29 subphase) and the **first subphase of the Phase 30 umbrella** — the umbrella stays 🟡 IN PROGRESS (30B + PDF table extraction + OCR remain). With **all of Phase 29 COMPLETE**, the evidence layer already carries **non-English primary sources** (several 29B.4 regulated-disclosure + event connectors emit `requires_translation` markers for French / German / Italian / Danish excerpts). Phase 30A lays the **language + translation foundation**, **OFF by default**, with the LLM-backed provider deliberately **not** the default. A new `app/services/sources/language.py` exposes `detect_language(text, hint)` for **en/fr/de/it/da** (generalizes the document-extractor stopword heuristic, adds Danish/Nordic) + `LANGUAGE_NAMES`/`language_name()`; `document_text_extractor._detect_language` now delegates (behaviour preserved). A new `app/services/sources/translation.py` defines `TranslationResult` + a `TranslationProvider` ABC with **`FakeTranslationProvider`** (the DEFAULT — an honest, clearly-marked placeholder that NEVER fabricates fluent English) and **`LLMTranslationProvider`** (composes the existing `get_llm_client()` — **no new host/model/secret**; bounded input AND output; **logs ONLY counts + language codes, never the prompt/original/translated text** — regression-tested), selected by `get_translation_provider(cfg, *, client)`. Every result carries a `MACHINE_TRANSLATION_WARNING` ("machine-assisted, NOT an official translation; human review required") + `needs_human_review=True`, is **bounded per-excerpt (never whole-document)**, and always preserves the **original text + source URL**. The single-company council (`_collect_translated_excerpts`, gated by `SOURCE_TRANSLATION_ENABLED`) surfaces bounded translations as `CouncilResult.translated_excerpts` metadata (empty `[]` when off — mirroring the macro/event precedent), and the final report gains an optional `translated_evidence` block (safety-scanned before validation). **Interpretation call (documented):** translated excerpts are council metadata + a report block (each retaining the original source URL as the citation of record), **not** injected into the single-company evidence pack; original evidence is untouched. Same discipline: evidence-first, citation-bound, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated. Backend **2264 pass / 12 skip / 0 fail** (+26 `test_phase30a_translation.py`), ruff clean, mypy `71` baseline (no new), security scan PASS (ib-security-agent — off-by-default, bounded, **text-free logging** on the LLM path, machine-assisted-not-official warning, no new credential/host/SSRF surface; pre-PR review APPROVED 10/10); 9 files (incl. tests). **Staging validation (2026-07-28) — OFF-state VALIDATED:** merged `fa3632a`, API `commit_sha=fa3632a` (3 stable polls), web unchanged (backend-only), no migration (head `011`), AUTH_TEST_MODE absent, publication admin-gated. **Registry unchanged** — **34 enabled / 2 scaffolded / 2 planned / 38 total** (30A adds no source), secret-free. **OFF-state no-regression:** a **POWL** company report council **8/8** agents, `translated_excerpts=[]`, **NO `translated_evidence` report block**, report body unchanged, `schema_valid` / `safety_valid` true, `publication_ready` false, `human_review_required` true. **Log hygiene:** 0 translation events since gated off (text-free by construction); AUTH_TEST_MODE absent. **⚠️ Minor note (operational, NOT a defect):** today's `docker.log` is not yet in the log download → the current-build live tail is a follow-up; mitigated by a clean API response surface + text-free logging by construction (only counts + language codes on the LLM path). `SOURCE_TRANSLATION_ENABLED` absent→**false** + `TRANSLATION_PROVIDER` absent→**fake** (dark); the 6 prior source/council flags remain ON. **KNOWN LIMITATION (recorded — happy-path NOT staging-demonstrable):** the non-English excerpt → machine-translation **happy-path is unit-fixture-proven, NOT staging-demonstrable** — the document-extraction pipeline on staging hits **scanned / English PDFs**, so **no non-English excerpts exist on staging to translate** (same no-OCR / scanned-PDF environmental condition first recorded in 29B.2); the OFF-state, metadata-only wiring, report-block render path, safety-scan, text-free logging and fake/LLM provider selection are all unit-test-covered; the end-to-end path becomes staging-demonstrable **once Phase 30B provides local-language sources**. **Interpretation call (documented):** translated excerpts are council metadata + a report block (each retaining the original `source_url` as citation of record), **not** injected into the single-company evidence pack — mirroring the macro/event precedent; LLM-backed translation is OFF by default (default provider `fake`). **Decision:** **`SOURCE_TRANSLATION_ENABLED` KEPT OFF on staging** — flipping it has no validation value (staging has no non-English extracted excerpts to translate); it becomes meaningful once **Phase 30B** provides local-language sources. Final staging flags (6 ON, `SOURCE_TRANSLATION_ENABLED` OFF/new): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true · `SOURCE_MACRO_ENABLED`=true · `SOURCE_EVENT_ENABLED`=true · **`SOURCE_TRANSLATION_ENABLED`=false (NEW, kept OFF)** · `TRANSLATION_PROVIDER`=`fake` (default). **Carry-forward limitations:** (1) the translation happy-path is unit-fixture-proven, not staging-demonstrable (extraction hits scanned/English PDFs → no non-English excerpts on staging; becomes demonstrable with 30B); (2) translated excerpts are council metadata + a report block, not evidence-pack citations (each retains the original `source_url`; original evidence untouched); (3) machine-assisted, NEVER official — `MACHINE_TRANSLATION_WARNING` + `needs_human_review` on every result, bounded per-excerpt, original always preserved; (4) current-build live log secret-scan not re-run (today's `docker.log` not yet in the download) — mitigated by clean API surface + text-free logging; (5) Azure OpenAI gpt-4.1-mini TPM quota remains a standing staging environmental limiter (OFF-state POWL run completed 8/8). Closure report: `docs/development/closures/phase-30a.md`. **The Phase 30 umbrella remains 🟡 IN PROGRESS. Next: Phase 30B — local-language evidence sources** (allowlisted connectors producing the non-English excerpts the 30A layer translates, integrated into the evidence budget; honest source gaps; allowlisted only — no broad web search unless explicitly reviewed + bounded), then **Phase 31 (source-aware research memo)**, then the wider Phase 30 PDF table extraction / OCR work.

### Previously: Phase 29D.3 Permit / Regulatory-Event Reference Connectors ✅ COMPLETE — merged + deployed + staging-validated (VALIDATED-WITH-ENVIRONMENTAL-NOTE) (PR #66 squash-merged to `main` at `d567019`; API `commit_sha=d567019` on 3 stable polls; web unchanged — backend-only, no web change; DB head `011`, **no migration, no new host/endpoint/secret, NO new flag** — reuses the existing OFF-by-default `SOURCE_EVENT_ENABLED`, already ON on staging from 29D.1). The **third and LAST** Phase 29D subphase — its merge + staging validation will **complete the entire Phase 29D umbrella** (procurement + patents + permits). Extends the reference-only **event-trigger** layer to **permits / regulatory-event venues** with **zero new wiring**: a new `PERMIT_SOURCES` table (into the combined `ALL_EVENT_SOURCES` = procurement + patents + permits) is served by the SAME generic `EventReferenceConnector` over three official public regulatory venues — **FERC** (`ferc.gov`, energy / grid / transmission / pipeline / LNG dockets & permits), **US NRC** (`nrc.gov`, nuclear reactor licensing / permits) and **US EPA** (`epa.gov`, environmental / emissions / industrial permitting) — all provider **`permits`** (a NEW `ProviderType.permits`), tier **T2**, `source_type="government_data"`, at fixed official public URLs (**no API key**), added as **NEW enabled** sources (not promoted from planned). A per-kind `_PERMIT_FLAVOR` makes permit references **purely thematic** (energy / grid / transmission / pipeline / nuclear / environmental / emissions / mining / lng / power-plant / permit / licensing — deliberately **excluding bare "industrial"** to avoid a GICS-Industrials substring collision with the 29D.1 defense path); each emits, per relevant theme, ONE bounded **T2 SOURCE REFERENCE** (fixed official public URL, no API key, **NO specific docket / case / permit number / applicant / decision / outcome / date**) plus an honest `data_not_sourced` gap plus an explicit disclaimer that **NO regulatory-outcome / approval / denial / materiality conclusion is drawn** — a permit reference is theme-scoped **WEAK CONTEXT** (never company-specific, never a catalyst / materiality / trade signal, never a candidate, never a recommendation). **Reference-only, network-free, OFF by default — reuses the existing `SOURCE_EVENT_ENABLED` flag (NO new flag)**, the `collect_theme_event_evidence` collector (iterating `ALL_EVENT_SOURCES`), the discovery-council `R#` citation path and the `industry_event_context` report block; dark-by-default byte-identical when off; `schema_valid`/`safety_valid` true, `publication_ready` false, `human_review_required` true; `/admin/*` OAuth-gated. **Also folds in the 29D.2 follow-up tidy:** `_event_discovery_facts` (`apps/api/app/services/llm/discovery_council.py`) now labels each discovery run-fact **per `provider_type`** (procurement byte-identical; patents → "patent office / index venue reference"; permits → "permit / regulatory-event venue reference") — resolving the 29D.2 cosmetic-mislabel follow-up. Registry → **34 enabled / 2 scaffolded / 2 planned / 38 total** (only `openbb` + local-language business press remain planned; SEDAR+/ASX scaffolds). **Deliberate deferral (carried from 29D.1/29D.2):** live permit / docket FETCH remains DEFERRED (reference-only) — the keyed FERC eLibrary / EPA / NRC ADAMS APIs are NOT used; a live permit/docket fetch is a documented follow-up. Backend **2238 pass / 12 skip / 0 fail** (+28; adjacent count tests updated to 34/38, no ripple), ruff clean, mypy `71` baseline (no new), security scan PASS (ib-security-agent; pre-PR review APPROVED 10/10); internal only, no BUY/SELL/HOLD/WATCH/target/fair-value/upside/recommendation, `human_review_required=true` / `publication_ready=false` unchanged. **Staging validation (2026-07-28) — VALIDATED-WITH-ENVIRONMENTAL-NOTE:** merged `d567019`, API `commit_sha=d567019` (3 stable polls), web unchanged (backend-only), no migration (head `011`), AUTH_TEST_MODE absent, publication admin-gated; **no app-setting flip** (`SOURCE_EVENT_ENABLED` already ON from 29D.1). **B — registry/health:** show `ferc` + `us_nrc` + `us_epa` all **enabled** (provider `permits`, T2), summary **34 enabled / 2 scaffolded / 2 planned / 38 total**, honest "permit / regulatory-event venue reference; live dockets not fetched; no regulatory-outcome conclusions; WEAK" notes, secret-free. **C — discovery:** a **"US nuclear power + grid"** run (uranium candidates **UEC / UUUU**) cites the permit sources as **reference-only WEAK** run-fact gaps ("no live FERC permit data included"); permits are **CONTEXT, NOT a candidate**. **D — safety:** `safety_valid` true, no fabricated dockets / outcomes, no regulatory-outcome conclusion, no recommendation; `publication_ready` false, `human_review_required` true. **E — per-provider label fix confirmed:** permit run-facts labelled "permit / regulatory-event venue reference" (not procurement / tender) — the folded-in 29D.2 patent-label tidy is done. **F — council:** 7/8 agents (the 1 failure = Azure OpenAI gpt-4.1-mini TPM throttling = **environmental**, not a code defect). **G:** logs clean (archive through 07-27; today's build runtime not yet archived → the API response surface was scanned clean; 29D.3 adds no logging), AUTH_TEST_MODE absent, 6 flags ON. **⚠️ Minor note (item G, operational, NOT a defect):** the current-build live log tail is a follow-up — the log archive lagged behind the 07-28 build; mitigated by a clean API surface + reference-only (no logging added); re-run the archived-log scan opportunistically. **Deliberate deferral (carried from 29D.1/29D.2):** live permit / docket FETCH remains DEFERRED (reference-only). **Decision:** `SOURCE_EVENT_ENABLED` **KEPT ON** (already ON; validation clean; reference-only / low-risk). Final staging flags KEPT (all **6** ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true · `SOURCE_MACRO_ENABLED`=true · `SOURCE_EVENT_ENABLED`=true. Closure report: `docs/development/closures/phase-29d3.md`. **This closure COMPLETES the entire Phase 29D umbrella** (29D.1 `a671e97` + 29D.2 `1c6b1c9` + 29D.3 `d567019`) — **and with it ALL OF PHASE 29 is now COMPLETE:** 29A source-registry + connector framework · 29B filings/regulator + primary docs/facts (29B / 29B.1 / 29B.2 / 29B.3) · 29B.4 EU/UK regulated disclosures (4A/4B/4C) · 29C macro/commodity/policy reference connectors (29C.1/29C.2/29C.3) · 29D event-trigger connectors (29D.1/29D.2/29D.3). **Next: Phase 30A — language detection + translation foundation** (a NEW capability area — per-evidence-item / per-excerpt language detection + translation-status fields on `EvidenceItem` + a translation-provider abstraction, OFF by default; French / German / Italian / Danish excerpts flagged `requires_translation` [already emitted by several 29B.4 / event connectors]; preserve original text + source URL; never translate whole documents; human-review warning for translated evidence), then the wider Phase 30 translation / local-language + PDF table extraction / OCR work.

### Previously: Phase 29D.2 Patent Event-Trigger Reference Connectors ✅ COMPLETE — merged + deployed + staging-validated (VALIDATED-WITH-ENVIRONMENTAL-NOTE) (PR #65 squash-merged to `main` at `1c6b1c9`; API `commit_sha=1c6b1c9` on 3 stable polls; web unchanged — backend-only, no web change; DB head `011`, **no migration, no new host/endpoint/secret, NO new flag** — reuses the existing OFF-by-default `SOURCE_EVENT_ENABLED`, already ON from 29D.1; the **second** Phase 29D subphase; the Phase 29D umbrella remains 🟡 IN PROGRESS). Extends the 29D.1 reference-only **event-trigger** layer to **patents** with **zero new wiring**: a new `PATENT_SOURCES` table (into the combined `ALL_EVENT_SOURCES`) is served by the SAME generic `EventReferenceConnector` over three official public patent venues — **Google Patents** (`patents.google.com`, `T5` aggregator index, provider `patents`), **USPTO** (`uspto.gov`, `T2` government, provider `patents`) and **EPO Espacenet** (`worldwide.espacenet.com`, `T2` government, provider `patents`) — all promoted PLANNED→**enabled**. A per-kind `_EventFlavor` makes patent references **purely thematic** (innovation / R&D / patent / IP / technology / semiconductor / pharma / battery / EV / materials), `source_type="government_data"`; each emits, per relevant theme, ONE bounded **T2/T5 SOURCE REFERENCE** (fixed official public URL, no API key, **NO specific patent number / title / inventor / assignee / claim / filing-or-grant date**) plus an honest `data_not_sourced` gap plus an explicit disclaimer that **NO legal / infringement / validity / patentability / ownership conclusion is drawn** — a patent reference is theme-scoped **WEAK CONTEXT** (never company-specific, never a catalyst / materiality / trade signal, never a candidate, never a recommendation). **Reference-only, network-free, OFF by default — reuses the existing `SOURCE_EVENT_ENABLED` flag (NO new flag)**, the `collect_theme_event_evidence` collector (now iterating `ALL_EVENT_SOURCES`), the discovery-council `R#` citation path and the `industry_event_context` report block; dark-by-default byte-identical when off; `schema_valid`/`safety_valid` true, `publication_ready` false, `human_review_required` true; `/admin/*` OAuth-gated. Registry → **31 enabled / 2 scaffolded / 2 planned / 35 total** (only `openbb` + local-language business press remain planned). **Deliberate deferral (carried from 29D.1):** live patent-filing FETCH remains DEFERRED (reference-only) — a Phase 29D follow-up. Backend **2210 pass / 12 skip / 0 fail** (+26; adjacent registry count assertions updated to 31 enabled/2 planned, no ripple), ruff clean, mypy `71` baseline (no new), security PASS (pre-PR review APPROVED 10/10). **Staging validation (2026-07-28) — VALIDATED-WITH-ENVIRONMENTAL-NOTE:** merged `1c6b1c9`, API `commit_sha=1c6b1c9` (3 stable polls), web unchanged (backend-only), no migration (head `011`), AUTH_TEST_MODE absent, publication admin-gated; **no app-setting flip** (`SOURCE_EVENT_ENABLED` already ON from 29D.1). **B — registry/health:** show `google_patents` (patents, T5), `uspto` (patents, T2) and `epo_espacenet` (patents, T2) all **enabled**, summary **31 enabled / 2 scaffolded / 2 planned / 35 total**, honest "patent venue reference; live filings not fetched; no legal / infringement conclusions; WEAK" notes, secret-free. **C — discovery:** a **"semiconductor innovation"** run (three real semiconductor candidates incl. **AMAT**) cites **Google Patents / USPTO / Espacenet (×11 each)** as **reference-only** `R#` run facts + honest "not fetched" gaps; patents are **CONTEXT, NOT a candidate** — proves ON-state citeability. **D — safety:** no fabricated patent numbers, no legal / infringement / validity conclusions, forbidden reco / valuation terms only negated; `safety_valid` true. **F — council:** `llm_used`, `completed_with_warnings` (partial completion = Azure OpenAI gpt-4.1-mini TPM throttling = **environmental**, not a code defect). **G:** 6 flags ON, AUTH_TEST_MODE absent, admin-gated. **⚠️ az-log-timeout note (operational, NOT a defect):** the staging **log secret-scan is UNKNOWN** because the `az` log download **timed out (>90s)** this session; mitigated because (1) the API response surface was scanned clean and (2) **29D.2 adds no logging** (reference-only, keyless — no new log line / secret path) — re-run the scan opportunistically. **⚠️ KNOWN FOLLOW-UP (non-blocking tidy, NOT a defect):** at the deployed SHA `1c6b1c9`, `_event_discovery_facts` (`apps/api/app/services/llm/discovery_council.py`) labels **patent** event references with generic **"procurement / tender venue reference (T2)"** boilerplate — a **cosmetic mislabel** for patent venues in the discovery-pack narration; **not a safety issue** (no fabrication, no legal conclusion — patent-specific honesty is carried by the item gap messages + the registry `reliability_note`). **Fix:** make `_event_discovery_facts` label **per `provider_type`** (procurement vs patents) — **to be folded into Phase 29D.3.** **Decision:** `SOURCE_EVENT_ENABLED` **KEPT ON** (already ON from 29D.1; validation clean; reference-only / low-risk). Final staging flags KEPT (all **6** ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true · `SOURCE_MACRO_ENABLED`=true · `SOURCE_EVENT_ENABLED`=true. Closure report: `docs/development/closures/phase-29d2.md`. **The Phase 29D umbrella remains 🟡 IN PROGRESS.** **Next: Phase 29D.3 permits / regulatory-event metadata — the LAST 29D subphase** (energy / mining / grid / industrial permits where safe; metadata-first; honest gaps; **no fake permits**; reuse the OFF-by-default `SOURCE_EVENT_ENABLED` layer — reference-only, no legal / regulatory conclusions; **also fold in** the `_event_discovery_facts` per-provider patent-label tidy), then **30 (translation / local-language + PDF table extraction / OCR)**.

### Previously: Phase 29D.1 Procurement / Tender Event-Trigger Reference Connectors ✅ COMPLETE — merged + deployed + staging-validated (PR #64 squash-merged to `main` at `a671e97`; API `commit_sha=a671e97` on 3 stable polls; web unchanged — backend-only, no web change; DB head `011`, **no migration, no new host/endpoint/secret**; two OFF-by-default flags `SOURCE_EVENT_ENABLED` / `SOURCE_EVENT_MAX_ITEMS`, **INDEPENDENT of the 29C `SOURCE_MACRO_ENABLED` flag**). The **first Phase 29D subphase** — establishes a NEW **event-trigger** evidence category (parallel to the 29C macro layer, **reference-only + OFF by default**). A new `EventReferenceConnector` (`sources/connectors/event_reference.py`) over two official public procurement/tender venues — **EU TED** (`ted.europa.eu`) + **USAspending.gov** (`usaspending.gov`) — implements the previously theme-dead `fetch_events` hook to emit, per relevant theme, ONE bounded **T2 SOURCE REFERENCE** (`source_type="government_data"` — deliberately NOT "government_contract"; `ProviderType` `procurement`; fixed official public URL, no API key, **NO specific tender / award / contractor / amount / contract-number / date**) plus an honest `data_not_sourced` gap ("live tenders/awards not fetched at report time"). Each item is a **WEAK** internal research-priority signal that carries `needs_human_review`, records freshness via `stale_after_days`, and is **NOT a materiality claim / candidate / catalyst / trade signal** — network-free, never a recommendation. `collect_theme_event_evidence(theme, region, cfg)` (`sources/event_evidence.py`) is DARK when `SOURCE_EVENT_ENABLED` is off and bounded by `SOURCE_EVENT_MAX_ITEMS`. The registry promotes `eu_ted` + `usaspending` PLANNED→**enabled** event-reference sources → **28 enabled / 2 scaffolded / 5 planned / 35 total** (SEDAR+/ASX scaffolds; `google_patents` / `uspto` / `epo_espacenet` + `openbb` + local-language press stay planned → 29D.2 / later). When on, the discovery council threads event references as citeable `R#` run facts (in `evidence_ids()`) + honest gaps, and the company report gains an OPTIONAL `industry_event_context` block (WEAK event CONTEXT — not company-specific, not a catalyst/materiality/trade signal, no figures); `CouncilResult.event_context` via `to_metadata_dict` (empty `[]` off); dark-by-default byte-identical when off. Same Phase 29B/29C discipline: **evidence-first, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside**; source registry + `EvidenceItem` tiers + explicit `SourceGap`s (honest gaps, never fabricated data); no broad / arbitrary web search; network-safe; OFF-by-default flags; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated. **Deliberate deferrals:** live EU TED / USAspending tender/award FETCH is DEFERRED (reference-only this subphase, a Phase 29D follow-up mirroring the 29B.4/29C deferrals — no API keys, no report-time network); full candidate-generation-from-events is DEFERRED (AC1 satisfied at the *context* level — candidates still come from the curated registry). Backend **2184 pass / 12 skip / 0 fail** (+34; new `test_phase29d1`; two adjacent registry count assertions updated to **28 enabled / 5 planned**, no functional ripple), ruff clean, mypy `71` baseline (no new), security scan PASS (ib-security-agent — reference-only, network-free, no API keys, `government_data` not `government_contract`, WEAK+needs-review, no fabricated awards, event flag independent of macro; pre-PR review APPROVED 10/10). **Staging validation (2026-07-28) — OFF-state VALIDATED + ON-state VALIDATED-WITH-ENVIRONMENTAL-NOTE:** merged `a671e97`, API `commit_sha=a671e97` (3 stable polls), web unchanged (backend-only), no migration (head `011`), AUTH_TEST_MODE absent, logs current-build clean, publication admin-gated. **OFF (default):** `/sources/registry` + `/sources/health` show **28 enabled / 2 scaffolded / 5 planned / 35 total** with `eu_ted` + `usaspending` enabled `procurement`/**T2**, the layer **fully dark at runtime** and **macro independence confirmed** (event flag off leaves the macro layer unchanged), no regression. **ON (human-approved `az` flip `SOURCE_EVENT_ENABLED=true`):** a **"defense procurement Europe"** discovery run (candidates **Thales / BAE / Saab / Leonardo / Rheinmetall**) cites **USAspending (×3) + "EU TED and USAspending.gov"** as **reference-only** `R#` run facts + an honest gap ("live tenders/awards not fetched; only references to official datasets"), treats events as **WEAK CONTEXT, NOT candidates**, safety valid — **proving ON-state citeability**. A company report is valid (**8/8** agents, `schema_valid` / `safety_valid` true, `publication_ready` false, `human_review_required` true), **but the `industry_event_context` + `industry_macro_context` blocks did NOT render for the tested European defense names** because their `free_real` profiles were **thin** (no sector/industry → empty theme → the collectors correctly stayed **dark**; the `event_context`/`macro_context` keys are **present-but-empty**) — a **data-thinness environmental condition, NOT a code defect** (the render path is **unit-test-covered** and ON-state citeability is proven by the discovery run). Councils **8/8** (no Azure OpenAI TPM failure this run); logs clean; publication admin-gated. **Decision:** `SOURCE_EVENT_ENABLED` **KEPT ON** on staging (validation clean; reference-only / low-risk; matches keeping `SOURCE_MACRO_ENABLED` / `SOURCE_CONNECTOR_ENABLED` / `SOURCE_DOCUMENT_EXTRACTION_ENABLED` on). Final staging flags KEPT (all **6** ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true · `SOURCE_MACRO_ENABLED`=true · **`SOURCE_EVENT_ENABLED`=true (NEW, kept ON)**. **⚠️ Carry-forward (KNOWN LIMITATION, NOT a defect — future refinement):** the company-report event / macro context blocks depend on a **non-thin company sector / industry classification** (`free_real`) to derive a theme; **thin / mock profiles → the collectors stay dark → the blocks don't render** (the `industry_event_context` / `industry_macro_context` keys are present-but-empty) — **same coarse / thin company→theme carry-forward first noted in 29C.2**; the citation mechanism + the registry are proven (OFF-state registry + the ON-state discovery run), so this is a company→theme derivation refinement, not a defect — refine it in a follow-up. **Carry-forward limitations:** (1) company→theme derivation from a thin / coarse company profile under-surfaces the event / macro context blocks in company reports (known limitation / future refinement, not a defect — see above); (2) no live tender / award figures — T2 SOURCE REFERENCE + honest `data_not_sourced` gap only (deferred, reference-only); (3) event references are theme / industry CONTEXT only — never company-specific, never a catalyst / materiality / trade signal, never a recommendation; (4) Azure OpenAI gpt-4.1-mini TPM quota remains a standing staging environmental limiter (no council-agent failure this run), not a defect. Closure report: `docs/development/closures/phase-29d1.md`. **The Phase 29D umbrella remains 🟡 IN PROGRESS** (29D.2 patents + 29D.3 permits / regulatory-event metadata remain). **Next: Phase 29D.2 patents** (promote the `google_patents` / `uspto` / `epo_espacenet` PLANNED rows — same reference-only, OFF-by-default event pattern; **NO legal / infringement / patent-validity conclusions**), then **29D.3 permits / regulatory-event metadata**, then **30 (translation / local-language + PDF table extraction / OCR)**.

### Previously: Phase 29C.3 Policy + Government Reference Connectors ✅ COMPLETE — merged + deployed + staging-validated (PR #63 squash-merged to `main` at `ad6dde5`; API `commit_sha=ad6dde5` on 3 stable polls; web unchanged — backend-only, no web change; DB head `011`, **no migration, no new host/endpoint, NO new flag** — reuses the existing OFF-by-default `SOURCE_MACRO_ENABLED`, already ON from 29C.1). **The third and LAST Phase 29C subphase — its closure COMPLETES the entire Phase 29C umbrella** (macro + commodity/energy + policy/government reference connectors). Extends the reference-only macro layer to **policy + government** context with **zero new wiring**. A new `POLICY_GOVERNMENT_SOURCES` table (added into the combined `ALL_MACRO_SOURCES`) is served by the SAME generic `MacroReferenceConnector` (`apps/api/app/services/sources/connectors/macro_reference.py`) over five official public policy/government publishers — **USTR / EU TARIC** (`ustr_taric`, `trade_policy`, T2; tariffs / trade / customs) and **UN Comtrade** (`un_comtrade`, `trade_policy`, T2; tariffs / trade / customs), both **promoted PLANNED→enabled**; plus three **new** sources **NATO defence expenditure** (`nato`, `trade_policy`, T2, `nato.int`; defense / military-spending / procurement / arms), **SIPRI military expenditure** (`sipri`, `trade_policy`, T3, `sipri.org`; defense / military-spending / procurement / arms) and **OECD** (`oecd`, `macro_statistics`, T2, `oecd.org`; subsidies / industrial-policy / state-aid / energy-transition / grid-investment). Each emits, for a relevant theme, ONE bounded **T2/T3 `macro_report` SOURCE REFERENCE** (fixed official public URL + which datasets it covers) **plus an honest `data_not_sourced` gap** — **no budget / spending-% / tariff-rate / subsidy figure or date is ever emitted; network-free; no API key**. `registry.py` promotes `ustr_taric` + `un_comtrade` PLANNED→**enabled** and adds `nato` / `sipri` / `oecd` (built from `ALL_MACRO_SOURCES` so registry + connectors never drift) → registry now **26 enabled / 2 scaffolded / 7 planned** (only SEDAR+/ASX scaffolds; USAspending / EU TED / OpenBB + patent rows stay planned → 29D; total **35**). The layer **reuses the existing `SOURCE_MACRO_ENABLED` flag** and the same theme collector (`collect_theme_macro_evidence` now iterates `ALL_MACRO_SOURCES`), the discovery-council `R#` citation path and the report `industry_macro_context` block; **dark-by-default** (flag off → discovery pack + report body byte-identical to Phase 29C.2). *(Note: `ProviderType` has no `government_data` member, so the government sources use the existing `trade_policy` / `macro_statistics` members.)* Same Phase 29B/29C discipline: **evidence-first, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside; prefer official / government sources**; wire through the existing **source registry + `EvidenceItem` tiers + explicit `SourceGap`s** (honest gaps, never fabricated data); **no broad / arbitrary web search**; network-safe; OFF-by-default flag; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated. **Deliberate deferral (carried from 29C.1/29C.2):** live policy / government FIGURE fetch is DEFERRED (reference-only) — a keyless official-data-API fetch (USTR-TARIC / UN Comtrade / NATO / SIPRI / OECD) is a documented follow-up. Backend **2150 pass / 12 skip / 0 fail** (new `test_phase29c3`; two stale `enabled==21` count assertions fixed to `26` for the five policy/government promotions, no functional ripple), ruff clean, mypy `71` baseline (no new), security scan PASS (pre-PR review APPROVED 10/10). **Staging validation (2026-07-27) — VALIDATED-WITH-ENVIRONMENTAL-NOTE:** merged `ad6dde5`, API `commit_sha=ad6dde5` (3 stable polls), web unchanged (backend-only), no migration (head `011`), AUTH_TEST_MODE absent, logs current-build clean, publication admin-gated; **no app-setting flip this subphase** (`SOURCE_MACRO_ENABLED` already ON from 29C.1). **B — registry/health:** show `ustr_taric` / `un_comtrade` / `nato` / `oecd` (T2) + `sipri` (T3) all **enabled** (provider `trade_policy` / `macro_statistics`), summary **26 enabled / 2 scaffolded / 7 planned / 35 total**, honest "policy / government reference only; live figures not fetched" notes, secret-free. **C — discovery:** a **defense-themed** run (candidates **Thales / BAE / Rheinmetall**) cites **NATO (×13) + USTR-TARIC (×3)** as macro `R#` run facts + honest gaps, treats macro as **CONTEXT not a candidate** — **SIPRI / OECD / UN-Comtrade did NOT surface for this specific theme** (coarse theme→source keyword map — see the carry-forward below; the citation mechanism is proven). **D — safety:** `safety_valid` true, **no fabricated figures**, **no recommendation language**. **E — discovery council:** **6/8** agents (the 2 failures = Azure OpenAI gpt-4.1-mini TPM throttling = **environmental**, not a code defect). **F:** AUTH_TEST_MODE absent, admin-gated, 5 flags ON, logs clean. **G — company render:** the company `industry_macro_context` render is inferred from **31 unit tests** + the proven reference-only behaviour (a fresh full-council company run was skipped to avoid Azure OpenAI TPM burn on an unchanged render path). **⚠️ Carry-forward (KNOWN LIMITATION, NOT a defect — future refinement):** coarse company / discovery theme→source keyword mapping means **not every specialist source surfaces for every theme** (in the defense run NATO + USTR-TARIC surfaced but SIPRI / OECD / UN-Comtrade did not); the **citation mechanism + the registry are proven** (validations B + C), so this is a mapping refinement, not a source defect — refine the theme→source keyword map in a follow-up. Final staging flags KEPT (all 5 ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true · `SOURCE_MACRO_ENABLED`=true. **Carry-forward limitations:** (1) coarse theme→source keyword mapping under-surfaces some specialist sources per theme (known limitation / future refinement, not a defect — see above); (2) no live policy / government figures — T2/T3 source reference + honest `data_not_sourced` gap only (deferred, reference-only); (3) policy / government is theme / industry CONTEXT only — never company-specific, never a catalyst, never a recommendation; (4) discovery-council partial completion under large real-data packs is an Azure OpenAI gpt-4.1-mini TPM environmental limit, not a defect. Closure report: `docs/development/closures/phase-29c3.md`. **This closure COMPLETES the entire Phase 29C umbrella** (29C.1 `a8ac580` + 29C.2 `80c8454` + 29C.3 `ad6dde5`). **Next: Phase 29D event-trigger connectors** (metadata-first — likely split 29D.1 procurement/tenders [`eu_ted` / `usaspending`], 29D.2 patents [`google_patents` / `uspto` / `epo_espacenet`], 29D.3 permits/regulatory-event metadata; honest gaps, no fake awards/contracts, no broad crawling, event = internal research-priority evidence only, no recommendation), then **30 (translation / local-language + PDF table extraction / OCR).**

### Previously: Phase 29C.2 Commodity + Energy Reference Connectors ✅ COMPLETE — merged + deployed + staging-validated (PR #62 squash-merged to `main` at `80c8454`; API `commit_sha=80c8454` on 3 stable polls; web unchanged — backend-only, no web change; DB head `011`, **no migration, no new host/endpoint, NO new flag** — reuses the existing OFF-by-default `SOURCE_MACRO_ENABLED`, already ON from 29C.1). The **second** Phase 29C subphase — extends the 29C.1 reference-only macro layer to **commodity + energy** with **zero new wiring**. A new `COMMODITY_ENERGY_SOURCES` table (plus a combined `ALL_MACRO_SOURCES`) is added to the SAME generic `MacroReferenceConnector` (`apps/api/app/services/sources/connectors/macro_reference.py`) over five official public agencies (all `commodity` provider type) — **USGS** (`usgs.gov`, T3; copper / lithium / rare-earths / critical minerals / cobalt / nickel / mining / uranium), **US EIA** (`eia.gov`, T2, **no API key**; uranium / nuclear / oil / gas / energy / electricity), **IEA** (`iea.org`, T3; energy / power-grid / nuclear / renewables / energy-transition), **IRENA** (`irena.org`, T3; renewables / solar / wind / hydrogen), and **ENTSO-E** (`transparency.entsoe.eu`, T3; power-grid / electricity / grid / transmission). Each emits, for a relevant theme, ONE bounded **T2/T3 `macro_report` SOURCE REFERENCE** (fixed official public URL + which datasets it covers) **plus an honest `data_not_sourced` gap** — **no tonnage / price / capacity / production / reserve figure or release date is ever emitted; network-free; no API key**. `registry.py` promotes the five commodity/energy rows from PLANNED to **enabled** (built from `ALL_MACRO_SOURCES` so registry + connectors never drift) → registry now **21 enabled / 2 scaffolded** (only SEDAR+/ASX remain; **9 planned**; total 32). The layer **reuses the existing `SOURCE_MACRO_ENABLED` flag** and the same theme collector (`collect_theme_macro_evidence` now iterates `ALL_MACRO_SOURCES`), the discovery-council `R#` citation path and the report `industry_macro_context` block; **dark-by-default** (flag off → discovery pack + report body byte-identical to Phase 29C.1). Same Phase 29B/29C discipline: **evidence-first, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside; prefer official / government sources**; wire through the existing **source registry + `EvidenceItem` tiers + explicit `SourceGap`s** (honest gaps, never fabricated data); **no broad / arbitrary web search**; network-safe; OFF-by-default flag; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated. **Deliberate deferral (carried from 29C.1):** live commodity / energy FIGURE fetch is DEFERRED (reference-only) — a keyless official-data-API fetch (USGS / EIA / IEA / IRENA / ENTSO-E) is a documented **29C follow-up**. Backend **2119 pass / 12 skip / 0 fail** (+27 net `test_phase29c2_commodity_energy_connectors.py`; adjacent count tests updated, no ripple), ruff clean, mypy `71` baseline (no new), security scan PASS (ib-security-agent — reference-only, network-free, no API keys incl. EIA, no fabricated figures; pre-PR review APPROVED 10/10). **Staging validation (2026-07-27) — VALIDATED-WITH-ENVIRONMENTAL-NOTE:** merged `80c8454`, API `commit_sha=80c8454` (3 stable polls), web unchanged (backend-only), no migration (head `011`), AUTH_TEST_MODE absent, logs current-build clean, publication admin-gated; **no app-setting flip this subphase** (`SOURCE_MACRO_ENABLED` already ON from 29C.1). **B — registry/health:** show `usgs` / `iea` / `irena` / `entsoe` (`commodity`, T3) + `eia` (`commodity`, T2) all **enabled**, summary **21 enabled / 2 scaffolded / 9 planned**, honest "reference only, no figures, no API key" notes, secret-free. **C — discovery:** a **uranium** run cites **US EIA + IEA** as macro `R#` references + honest gaps, **0 figure tokens**, returns candidates (`CCJ` / `UEC` / `UUUU` / `FCX`), treats macro as **CONTEXT not a candidate**, discovery council **8/8** — **proves the new specialist sources surface and are citeable**. **D — company reports (PARTIAL, environmental):** two reports (**Cameco**, **UEC**) render the `industry_macro_context` block reference-only (no figures), `macro_context` non-empty, `schema_valid` / `safety_valid` true, `publication_ready` false, `company_ir` / SEC present — **but the NEW specialist sources did NOT surface for these two names** because `free_real` classifies them `sector="Materials"` / no-industry, so the coarse company→theme derivation matched only the pre-existing 29C.1 World Bank 'Pink Sheet' keywords, not the new specialist keywords (see the carry-forward below); NEW-source surfacing is proven via registry (B) + discovery-council (C) + the unit-test-covered render. **E — safety:** no fabricated figures; forbidden reco / valuation terms only inside negated disclaimers. **F — company council:** **7/8** agents (the 1 failure = Azure OpenAI gpt-4.1-mini TPM throttling = **environmental**, not a code defect). **G:** AUTH_TEST_MODE absent, admin-gated, 5 flags ON. **⚠️ Carry-forward (KNOWN LIMITATION, NOT a defect — future refinement):** the **company-report** macro theme is derived from the company's **sector / industry**, which is **coarse for commodity / energy names** (`free_real` frequently returns `sector="Materials"` / no-industry), so specialist commodity/energy sources (USGS / EIA / IEA) may **UNDER-SURFACE in company reports** until the company→theme derivation is improved (e.g. map `sector + ticker/name` → the right specialist). The sources are **wired correctly and work** at the discovery-council + registry level (proven B + C, render unit-test-covered). Final staging flags KEPT (all 5 ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true · `SOURCE_MACRO_ENABLED`=true. **Carry-forward limitations:** (1) coarse company→theme derivation under-surfaces specialist sources in company reports for `Materials`-labelled issuers (known limitation / future refinement, not a defect — see above); (2) no live commodity / energy figures — T2/T3 source reference + honest `data_not_sourced` gap only (deferred, reference-only); (3) commodity / energy is theme / industry CONTEXT only — never company-specific, never a catalyst, never a recommendation; (4) company council partial completion under large real-data packs is an Azure OpenAI gpt-4.1-mini TPM environmental limit, not a defect. Closure report: `docs/development/closures/phase-29c2.md`. **Next: Phase 29C.3 policy + government — the LAST 29C subphase** (defense / NATO spending, tariffs, subsidies, industrial policy, grid investment, energy transition — prefer official / government sources: USTR-TARIC / USAspending / EU TED / UN Comtrade; same reference-only, OFF-by-default pattern), then **30 (translation / local-language + PDF table extraction / OCR)**.

### Previously: Phase 29C.1 Macro Baseline Evidence Connectors ✅ COMPLETE — merged + deployed + staging-validated (PR #61 squash-merged to `main` at `a8ac580`; API `commit_sha=a8ac580` on 3 stable polls; web unchanged — backend-only, no web change; DB head `011`, no migration, no new host/endpoint; two OFF-by-default flags `SOURCE_MACRO_ENABLED` / `SOURCE_MACRO_MAX_ITEMS`). The **first** Phase 29C subphase and the platform's first **macro / thematic** evidence layer — **reference-only + OFF by default**. A generic `MacroReferenceConnector` (`apps/api/app/services/sources/connectors/macro_reference.py`) over five official public macro sources — FRED (`fred.stlouisfed.org`), IMF, Eurostat, World Bank Commodity 'Pink Sheet', national statistics offices / central banks — implements `fetch_macro_context` (previously a dead hook) to emit, for a relevant theme/region, ONE bounded **T2 `macro_report` SOURCE REFERENCE** (fixed public token-free landing URL + which indicators the dataset covers) **plus an honest `data_not_sourced` gap** ("live figures not fetched at report time"). **No indicator value / index level / release date / forecast is ever emitted; network-free; no API key** (FRED-style keys deliberately not introduced). A theme collector `collect_theme_macro_evidence(theme, region, cfg)` (`sources/macro_evidence.py`) is dark when `source_macro_enabled` is False and bounded by `source_macro_max_items` (default 3). `registry.py` promotes the five macro sources from PLANNED to **enabled** reference-only macro sources (built from a single `MACRO_SOURCES` table) → registry now **16 enabled / 2 scaffolded** (only SEDAR+/ASX remain; total 32). When the flag is on, the **discovery council** cites macro references as `R#` run facts (+ honest gaps) and the **company report** gains an OPTIONAL `industry_macro_context` block (beside `industry_context_events`), each item labelled macro CONTEXT with an honest note that it is **NOT company-specific evidence and never a catalyst**, carrying no figures; `CouncilResult.macro_context` surfaces via `to_metadata_dict` (empty `[]` when off, mirroring `primary_documents`). `schema_valid` / `safety_valid` stay true, `publication_ready` false, `human_review_required` true; **dark-by-default** — with the flag off the discovery pack + report body are **byte-identical** to Phase 29B. **Deliberate deferral (documented, carried into 29C):** live macro-FIGURE fetch is DEFERRED (reference-only), mirroring the 29B.4 regulator content-fetch deferral — no API keys, no report-time network; a keyless official-data-API fetch (World Bank / Eurostat / IMF etc.) is a documented **29C follow-up**. Same Phase 29B discipline: **evidence-first, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside**; **prefer official / government sources**; wire through the existing **source registry + `EvidenceItem` tiers + explicit `SourceGap`s** (honest gaps, never fabricated data); **no broad / arbitrary web search**; network-safe (allowlisted hosts, SSRF-safe, bounded); OFF-by-default flags; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated; human approval before publication. All seven 29C.1 acceptance criteria satisfied (discovery cites theme macro; reports include industry-context evidence; evidence budget handles macro packs; no reco/valuation; no fabricated macro facts; source gaps explicit). Backend **2092 pass / 12 skip / 0 fail** (+21 net `test_phase29c1`; adjacent registry/source-count tests updated for the five macro promotions, no ripple), ruff clean, mypy `71` baseline (no new), security scan PASS (ib-security-agent — network-free, no fabricated macro figure/index/date/forecast, fixed public token-free URLs, honest gaps, OFF-by-default, no reco/valuation; pre-PR review APPROVED 10/10). **Staging validation (2026-07-27) — OFF-state VALIDATED (clean) + ON-state VALIDATED-WITH-ENVIRONMENTAL-NOTE:** merged `a8ac580`, API `commit_sha=a8ac580` (3 stable polls), web unchanged (backend-only), no migration (head `011`), AUTH_TEST_MODE absent, logs current-build clean, publication admin-gated. **OFF (default):** `/sources/registry` + `/sources/health` show **16 enabled / 2 scaffolded** with the five macro sources enabled `T2_regulator_or_gov` reference-only, the layer **fully dark at runtime** (byte-identical to Phase 29B), no regression. **ON (human-approved `az` flip `SOURCE_MACRO_ENABLED=true`):** the **SCCO (Southern Copper)** company report gains an `industry_macro_context` block (`world_bank_pink_sheet`, T2, token-free URL, **NO figures / dates**, honest "not company-specific / not a catalyst" note), `llm_council.macro_context` non-empty, `schema_valid=true`, `safety_valid=true`, `publication_ready=false`, `human_review_required=true`; a **"copper mining"** discovery run cites macro references (World Bank ×3, Pink Sheet ×2) + honest gaps, returns a candidate, and treats macro as **CONTEXT not a candidate**; safety clean (no fabricated figures; forbidden reco/valuation terms only inside negated disclaimers); council discovery **8/8**, company report **7/8** (the 1 failure = Azure OpenAI gpt-4.1-mini TPM throttling = **environmental**, not a code defect). **Architecture note (verified):** macro evidence is **theme-scoped** (surfaces in reports + discovery), **NOT issuer-scoped** — correctly **absent** from the per-issuer company evidence-preview. **Decision:** `SOURCE_MACRO_ENABLED` **KEPT ON** on staging (validation clean; reference-only / low-risk; matches keeping `SOURCE_CONNECTOR_ENABLED` + `SOURCE_DOCUMENT_EXTRACTION_ENABLED` on). Final staging flags KEPT (all ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true · **`SOURCE_MACRO_ENABLED`=true (NEW, kept ON)**. **Carry-forward limitations:** (1) no live macro figures — T2 source reference + honest `data_not_sourced` gap only (deferred, reference-only; keyless official-data-API fetch is a documented 29C follow-up); (2) macro is theme/industry CONTEXT only — never company-specific, never a catalyst, never a recommendation; (3) company council partial completion under large real-data packs is an Azure OpenAI gpt-4.1-mini TPM environmental limit, not a defect. Closure report: `docs/development/closures/phase-29c1.md`. **Next: Phase 29C.2 commodity + energy** (copper / lithium / rare-earths / uranium / nuclear / power-grid / electricity-demand; official sources USGS / EIA / IEA / IRENA / ENTSO-E / World Bank Pink Sheet — same reference-only, OFF-by-default pattern; several of these sources are already registered PLANNED/enabled), then **29C.3 policy + government** (USTR-TARIC / USAspending / EU TED / UN Comtrade), then **30 (translation / local-language + PDF table extraction / OCR)**.

### Previously: Phase 29B.4C Swiss / Nordic / Germany Regulated-Disclosure Connectors ✅ COMPLETE — merged + deployed + staging-validated (PR #60 squash-merged to `main` at `de126ee`; API `commit_sha=de126ee` on 3 stable polls; web unchanged at `793e0a7` — backend-only, no web change; DB head `011`, no migration, no new flag/host/endpoint). **This subphase completes the entire Phase 29B.4 umbrella** (EU/UK regulated-disclosure connectors: 4A UK FCA NSM + 4B Euronext + 4C Swiss/Nordic/Germany). Adds **three new dedicated regulator connectors** following the same **promote-scaffold→connector / T2 venue *reference* + honest `primary_filing_unavailable` content gap, network-free at report time, verified-issuer-gated** pattern proven in 29B.4A (UK) and 29B.4B (Euronext) — resolved via `verified_issuer_sources`, **never a fabricated filing/headline/date/notice number**: **`DeutscheBoerseConnector`** (`apps/api/app/services/sources/connectors/deutsche_boerse.py`, promotes the `deutsche_boerse` scaffold) — `SAP.DE` SAP (Germany / Xetra) → ONE bounded **T2 reference** to the German regulated-information venue (Bundesanzeiger / Deutsche Börse / BaFin) at the fixed public URL `https://www.bundesanzeiger.de` + German **`requires_translation`** (`GapType.translation_required`) + honest content gap; **`NordicDisclosuresConnector`** (`connectors/nordic_disclosures.py`, promotes the `nordic_disclosures` scaffold) — `PNDORA.CO` Pandora (Denmark / Nasdaq Copenhagen) → **T2 reference** (Nasdaq Nordic company news `https://www.nasdaqomxnordic.com/news/companynews` / Finanstilsynet) + Danish **`requires_translation`** (generalizes to the ST / HE / OL Nordic venues) + honest content gap; and **`SixSwissConnector`** (`connectors/six_swiss.py`, a **NEW `six_swiss` source built from scratch — no Swiss scaffold existed**, resolving the honest Swiss/SIX gap the 29B.4B plan flagged) — `CFR.SW` Richemont + `UHR.SW` Swatch on SIX (SW / VX) → **T2 reference** (SIX Swiss Exchange / SIX Exchange Regulation official notices) + honest content gap, with **NO `requires_translation` claim** — Switzerland is multilingual and major issuers publish English, so only a neutral DE/FR/IT multilingual note in `warnings` (honest, not a translation claim). `company_evidence.py` extends the exchange/country→regulator mapping (DE/Xetra/Frankfurt+Germany → `deutsche_boerse`; CO/Nasdaq-Copenhagen+Denmark → `nordic_disclosures`; SW/VX/SIX+Switzerland → `six_swiss`) + `REGULATOR_REFERENCE_IDS` + tightened `_relevant_scaffold_ids`; `registry.py` promotes `deutsche_boerse` + `nordic_disclosures` out of the scaffold table and registers `six_swiss` as a **new enabled `regulator` / `T2_regulator_or_gov`** source, so `/sources/registry` + `/sources/health` show all three enabled and the summary becomes **11 enabled / 2 scaffolded** (remaining scaffolds: **SEDAR+, ASX only**). **Deferral carried forward (from 29B.4A/4B, now across the whole umbrella):** the live regulator-venue CONTENT fetch stays deferred (reference-only) — the honest T2 venue reference + explicit content gap is the shipped posture. No recommendation/rating/BUY-SELL-HOLD-WATCH/price-target/fair-value/upside; `human_review_required=true` / `publication_ready=false` unchanged; no publish route; no auth change. Backend **2071 pass / 12 skip / 0 fail** (+16 `test_phase29b4c`; adjacent scaffold-count tests updated for the three promotions, no ripple), ruff clean, mypy `71` baseline (no new), security scan PASS (ib-security-agent — network-free, no fabricated filings, fixed public HTTPS venue URLs with no query/secret, verified-issuer-gated, honest gaps, `six_swiss` carries no translation claim; pre-PR review APPROVED 10/10). **Staging validation (2026-07-27) — VALIDATED (full):** merged `de126ee`, API `commit_sha=de126ee` (3 stable polls), web unchanged `793e0a7` (no web change), no migration (head `011`), AUTH_TEST_MODE absent, logs current-build clean (the sole `api_token=` match is the known 2026-07-22 historical pre-hotfix leak, not a current-build regression), publication admin-gated; `/sources/registry` + `/sources/health` show `deutsche_boerse` + `nordic_disclosures` + `six_swiss` all enabled `regulator`/`T2_regulator_or_gov`, summary **11 enabled / 2 scaffolded** (only SEDAR+ + ASX remain), honest "content not fetched at report time" notes, `six_swiss` asserts **NO** translation, secret-free; `SAP.DE` → SAP with a `deutsche_boerse` **T2 venue reference** (`bundesanzeiger.de`) + German `requires_translation` + honest content gap, `company_ir` evidence still present; `PNDORA.CO` → Pandora with a `nordic_disclosures` **T2 venue reference** (`nasdaqomxnordic.com`) + Danish `requires_translation` + honest content gap, `company_ir` present; `CFR.SW` + `UHR.SW` → Richemont / Swatch with a `six_swiss` **T2 venue reference**, `requires_translation`=**false** (no `six_swiss` translation gap — the `translation_required` gap that is present is the **pre-existing `company_ir`'s**, honestly attributed), `company_ir` present; `BA.LSE` still maps to `uk_fca_nsm` with **no** DE/Nordic/Swiss item (BA → BAE Systems, **never Boeing**), `MC.PA` still maps to `euronext` with **no** DE/Nordic/Swiss leakage; AAPL guardrail — the three new connectors → `source_not_eligible`, **0 items** for a US issuer; `schema_valid=true`, `safety_valid=true`, `publication_ready=false`, `human_review_required=true`. Final staging flags KEPT (all ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true. **Carry-forward limitations:** (1) DE / Nordic / Swiss (and Euronext, UK FCA NSM) T1 filing CONTENT is not fetched — venue *reference* + honest content gap only (deferred, reference-only across all 29B.4 connectors); (2) SEDAR+ / ASX remain scaffolded (deferred to a later batch); (3) no local-language translation yet (German/Danish/other non-English flagged `requires_translation`, pending Phase 30; Switzerland carries only a neutral multilingual note, not a translation claim). Closure report: `docs/development/closures/phase-29b4c.md`. **The Phase 29B.4 umbrella is now COMPLETE** (4A `5138725` + 4B `1d97612` + 4C `de126ee`). **Next: Phase 29C (macro/commodity/policy evidence connectors — likely split 29C.1 macro baseline / 29C.2 commodity+energy / 29C.3 policy+government), then 30 (translation/local-language + PDF table extraction / OCR).**

### Previously: Phase 29B.4B Euronext Regulated-Disclosure Connector ✅ COMPLETE — merged + deployed + staging-validated (PR #59 squash-merged to `main` at `1d97612`; API `commit_sha=1d97612` on 3 stable polls; web unchanged at `793e0a7` — backend-only, no web change; DB head `011`, no migration, no new flag/host/endpoint). Applies the **promote-scaffold→connector pattern proven in 29B.4A** to Euronext: promotes the `euronext_regulated_info` scaffold to a dedicated `EuronextRegulatedConnector` (`apps/api/app/services/sources/connectors/euronext_regulated_info.py`) for verified Euronext issuers — Euronext Paris FR (`MC.PA` LVMH, `RMS.PA` Hermès, `KER.PA` Kering) and Euronext Amsterdam NL (`ASML.AS` ASML), resolved via `verified_issuer_sources`, **never a fabricated filing/headline/date/notice number** — emitting ONE bounded **T2 regulator-transport SOURCE REFERENCE** to the Euronext regulated-information venue + country regulator (AMF France / AFM Netherlands) at the fixed public URL `https://www.euronext.com/en/regulated-information` **plus an honest `primary_filing_unavailable` content gap**, **network-free at report time**. Extends `regulator_connector_for()` (Euronext Paris PA/FR + Amsterdam AS/NL → `euronext_regulated_info`; added to `REGULATOR_REFERENCE_IDS`) and tightens `_relevant_scaffold_ids` (UK still `uk_fca_nsm`; DE unchanged); `registry.py` moves `euronext_regulated_info` out of the scaffold table and registers it enabled `regulator` / `T2_regulator_or_gov` (registry now **8 enabled / 4 scaffolded**: SEDAR+/ASX/Deutsche Börse/Nordic). French-jurisdiction issuers (MC/RMS/KER) are **flagged `requires_translation`** (`GapType.translation_required`; pending Phase 30 — no local-language ingestion this subphase, **not** a claim of official translation). Unresolvable / non-Euronext issuers → honest `source_not_eligible` gap. **Deliberate deferral (live-content fetch, carried from 29B.4A):** the live regulator-venue CONTENT fetch is NOT built this subphase — a bounded server-side fetch reads essentially nothing while adding external-regulator fetch surface; the honest T2 reference + explicit content-gap is the shipped posture, and live content fetch remains a future 29B.4 follow-up. No recommendation/rating/BUY-SELL-HOLD-WATCH/price-target/fair-value/upside; `human_review_required=true` / `publication_ready=false` unchanged; no publish route; no auth change. Backend **2058 pass / 12 skip / 0 fail** (+12 `test_phase29b4b_euronext_disclosures.py`; +1 regression fix in `test_phase29b1` `test_26_27` to allow the promoted regulator-reference source_ids alongside `company_ir`), ruff clean, mypy `71` baseline (no new), security scan PASS (ib-security-agent — network-free, no fabricated filings, plain public HTTPS venue URL, verified-issuer-gated, honest gaps; pre-PR review APPROVED 10/10). **Staging validation (2026-07-27) — VALIDATED:** merged `1d97612`, API `commit_sha=1d97612` (3 stable polls), web unchanged `793e0a7` (no web change), no migration (head `011`), AUTH_TEST_MODE absent, logs clean (no secret leaks), publication admin-gated; `/sources/registry` + `/sources/health` show `euronext_regulated_info`=enabled `regulator`/`T2_regulator_or_gov`, **8 enabled / 4 scaffolded**, honest "content not fetched at report time" note, secret-free; `MC.PA` → LVMH with a Euronext **T2 venue reference** + honest `primary_filing_unavailable` gap + `requires_translation`=true (French) and an explicit `translation_required` gap, `company_ir` evidence still present; `ASML.AS` → ASML with a Euronext **T2 venue reference** **without** `requires_translation` (NL/English), `company_ir` present; `BA.LSE` still maps to `uk_fca_nsm` with **no** Euronext item, BA → BAE Systems **never Boeing** (Phase 29B.4A unaffected); AAPL/AMAT carry **no** Euronext item, SEC + `company_ir` unchanged; `CFR.SW` carries **no** Euronext mention and an honest not-eligible (`source_not_eligible`) gap; `schema_valid=true`, `safety_valid=true`, `publication_ready=false`, `human_review_required=true`. **Honest scoping note:** the connector delta was directly observed via authed `evidence-preview` on all six issuers (MC.PA / ASML.AS / BA.LSE / AAPL / AMAT / CFR.SW); the report-body / safety / publication fields were validated by the **untouched-code invariant** (this PR's diff touches only the connector + wiring + registry, and 29B.4A already proved a regulator reference flows into a full council FINAL report via the identical path) — fresh full-council runs were **deliberately skipped** to avoid Azure OpenAI TPM burn re-testing unchanged code. Final staging flags KEPT (all ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true. **Carry-forward limitations:** (1) Euronext (and UK FCA NSM) T1 filing CONTENT is not fetched — venue *reference* + honest content gap only (deferred, see above); (2) SEDAR+ / ASX / Deutsche Börse / Nordic remain scaffolded (4C covers Swiss / Nordic / Germany — note the Swiss/SIX honest gap: no scaffold exists to promote); (3) no local-language translation yet (FR/other non-English flagged `requires_translation`, pending Phase 30). Closure report: `docs/development/closures/phase-29b4b.md`. **Next: 29B.4C (Swiss / Nordic / Germany — `UHR.SW` Swatch / `CFR.SW` Richemont / `PNDORA.CO` Pandora / `SAP.DE` SAP), then 29C (macro/commodity/policy), 30 (translation/local-language + PDF table extraction / OCR).**

### Previously: Phase 29B.4A UK FCA NSM/RNS Regulated-Disclosure Connector ✅ COMPLETE — merged + deployed + staging-validated (PR #58 squash-merged to `main` at `5138725`; API `commit_sha=5138725` on 3 stable polls; web unchanged at `793e0a7` — backend-only, no web change; DB head `011`, no migration, no new flag/host/endpoint). Promotes the `uk_fca_nsm` regulator entry from a generic `ScaffoldConnector` to a dedicated `UkFcaNsmConnector` (`apps/api/app/services/sources/connectors/uk_fca_nsm.py`): for a verified UK-regulated LSE issuer (`BRBY.LSE` Burberry, `BA.LSE` BAE Systems — resolved via `verified_issuer_sources`, **never Boeing / SEC**) it emits ONE bounded **T2 regulator-transport SOURCE REFERENCE** citing the issuer's FCA National Storage Mechanism / RNS venue (fixed public NSM URL — no query, **no fabricated filing / headline / date / RNS number**) **plus an honest `primary_filing_unavailable` gap** that the T1 filing CONTENT is not fetched at report time (**network-free** at report time). Adds an explicit exchange/country→regulator mapping (`regulator_connector_for()` in `company_evidence.py`: LSE/GB → `uk_fca_nsm`) and tightens `_relevant_scaffold_ids` so a UK/LSE issuer maps to `uk_fca_nsm` **only** (DE/FR unchanged). `registry.py` removes `uk_fca_nsm` from the scaffold table and registers it as an **enabled `regulator` / `T2_regulator_or_gov`** source (honest reliability note "T1 content not fetched at report time — Phase 29B.4 follow-up"), so `/sources/registry` + `/sources/health` show it as an enabled reference connector (was `scaffolded`); the scaffold set is now **5** (SEDAR+/ASX/Euronext/Deutsche Börse/Nordic) — since reduced to **4** by Phase 29B.4B, which promoted Euronext. **Deliberate deferral (live-content fetch):** the live FCA-NSM CONTENT fetch is NOT built this subphase — the NSM is a JS SPA, so a bounded server-side fetch reads essentially nothing (mostly honest gaps) while adding external-regulator fetch surface; the honest T2 reference + explicit content-gap is the shipped posture, and live content fetch is a future 29B.4 follow-up. All 8 phase acceptance criteria satisfied (allowlisted/bounded, no arbitrary fetch, no fake filings, BA≠Boeing, company IR intact, CFR unchanged, schema/safety valid, explicit gaps). No recommendation/rating/BUY-SELL-HOLD-WATCH/price-target/fair-value/upside; `human_review_required=true` / `publication_ready=false` unchanged; no publish route; no auth change. Backend **2046 pass / 12 skip / 0 fail** (+12 `test_phase29b4a_uk_fca_disclosures.py`; updated registry/filing-scaffold tests for the promotion), ruff clean, mypy `71` baseline (no new), security scan PASS (ib-security-agent — network-free, no fabricated filings, plain public HTTPS venue URL, BA→BAE never Boeing, honest gaps; pre-PR review APPROVED 10/10). **Staging validation (2026-07-27) — VALIDATED (clean):** merged `5138725`, API `commit_sha=5138725` (3 stable polls), web unchanged `793e0a7` (no web change), no migration (head `011`), AUTH_TEST_MODE absent, logs clean (no secret leaks), publication admin-gated; `/sources/registry` + `/sources/health` show `uk_fca_nsm`=enabled `regulator`/`T2_regulator_or_gov`, scaffolded set = 5, honest "content not fetched at report time" note, secret-free; `BA.LSE` → BAE Systems with a FCA NSM **T2 venue reference** + honest `primary_filing_unavailable` gap, `company_ir` evidence still present, boeing=0 / sec.gov=0 / RNS number=0 / no fabricated filing, `schema_valid=true`, `safety_valid=true`, `publication_ready=false`, `human_review_required=true`, 8/8 council agents; `BRBY.LSE` same honest posture; AAPL/AMAT carry no UK item and are unchanged (8/8); CFR.SW carries no UK mention, an honest not-eligible gap and unchanged `company_ir` evidence. Final staging flags KEPT (all ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true. **Carry-forward limitations:** (1) UK FCA NSM T1 filing CONTENT is not fetched — venue *reference* + honest content gap only (deferred, see above); (2) SEDAR+ / ASX / Deutsche Börse / Nordic remain scaffolded (Euronext promoted in Phase 29B.4B; 4C covers Swiss / Nordic / Germany). Closure report: `docs/development/closures/phase-29b4a.md`. **Next: 29B.4B (Euronext — MC.PA/RMS.PA/KER.PA/ASML.AS), then 4C (Swiss/Nordic/Germany), 29C (macro/commodity/policy), 30 (translation/local-language + PDF table extraction / OCR).**

### Previously: Phase 29B.3 Primary-Fact Integration into Reports + Quality Gates ✅ COMPLETE — merged + deployed + staging-validated (PR #57 squash-merged to `main` at `29f4a84`; API `commit_sha=29f4a84` on 3 stable polls; web unchanged at `793e0a7` — backend-only, no web change; DB head `011`, no migration, no new flag). Integrates the high-confidence primary facts parsed in Phase 29B.2 into the final report and its quality gates: a bounded, **structured** `PrimaryFactRef` on `EvidenceItem` (no raw excerpt / document text) is attached to each `company_ir_financial_fact` (T1) item; the council persists **only high-confidence** facts **metadata-only** under `source_summary_json.llm_council.primary_facts` while the safety-scanned report body (`to_report_dict`) stays unchanged; the final report gains real `T1_primary_filing` datapoints (`<field>_primary_filing` in the Financial Snapshot **alongside** the preserved T5 EODHD values, a `reporting_currency` override + `fiscal_year`/`employees` in Company Identity, an `extracted_primary_facts` block in Source-Quality Review — each with its own `source_url` + page/excerpt provenance + `needs_human_review`), and the T1/T2 human-review checklist item is recomputed post-council to complete **ONLY** on a genuine high-confidence T1 fact; the strict-schema completer maps a genuine T1 fact to a properly-sourced datapoint (never `not_sourced` / "internal analysis snapshot") while **refusing a non-USD revenue fact into the USD `revenue_ttm_usd_m` field (no currency conversion)**. `publication_ready` stays false, `human_review_required` stays true, `research_complete` stays honest; no recommendation/rating/BUY-SELL-HOLD-WATCH/price-target/fair-value/upside; no publish route; no auth change. **Caveat 1 — staging 0-facts:** because Phase 29B.2 does no OCR, the only live verified-issuer reports reachable on staging are scanned / index-only PDFs → **0 facts materialize**, so `primary_facts` and the `*_primary_filing` datapoints render **present-but-empty** on live reports; the integrations are proven by unit fixtures and will light up once digital-text (non-scanned) primary sources exist. **Caveat 2 — capability-only uplift:** the scoring + research-completeness credit for primary facts is unit-tested but **NOT wired to a production caller** this phase (scoring runs pre-council, before facts exist), so there is **no live numeric uplift** yet. Backend **2033 passing** (+31 Phase 29B.3, 12 skipped, 0 fail), ruff clean, no new mypy over the ~71 baseline, security scan PASS (ib-security-agent 8/8; ib-pr-review-agent APPROVED 8/8 with currency-guard hardening — explicit-USD + explicit-millions required before writing `revenue_ttm_usd_m`). **Staging validation (2026-07-27) — VALIDATED-WITH-ENVIRONMENTAL-NOTE (honest-empty + no regression):** merged `29f4a84`, API `commit_sha=29f4a84` (3 stable polls), web unchanged `793e0a7` (no web change), no migration (head `011`), AUTH_TEST_MODE absent, logs clean, publication admin-gated; CFR.SW full analysis = final report, `llm_used`, 8/8 agents, `schema_valid=true`, `safety_valid=true`, `human_review_required=true`, `publication_ready=false`, `research_complete=false`, `primary_facts=0`, **no** `extracted_primary_facts` block, **no** populated `*_primary_filing` datapoints (0 facts — every reachable issuer report is scanned/no-OCR), T1/T2 checklist honestly not-completed; AAPL/AMAT no regression (partial agent completion = Azure OpenAI gpt-4.1-mini TPM throttling = environmental, not a defect); discovery regression PASS with **no score inflation** (confirms the scoring/completeness credit is capability-only / not wired to a production caller). Final staging flags KEPT (all ON): `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true. **Carry-forward limitations (Phase 29B.4 candidates):** (1) the facts-present happy path is proven by **unit fixtures only**, not live staging, because the extractor reaches only scanned/index-only PDFs (no-OCR) → 0 facts; it will light up with digital-text primary sources / OCR / future SEC full-text; (2) the scoring/completeness numeric uplift is **capability-only** (unit-tested optional param, not wired to a production caller). Closure report: `docs/development/closures/phase-29b3.md`. **Next: 29B.4 (live EU/UK regulated-disclosure connectors — start with subphase 4A: UK FCA NSM/RNS for BRBY.LSE + BA.LSE), then 29C (macro/commodity/policy), 30 (translation/local-language + PDF table extraction / OCR).**

### Previously: Phase 29B.2 Annual-Report Document Extraction + Primary-Fact Parsing ✅ (PR #56 merged + deployed + staging-validated at `793e0a7`; API + Web `commit_sha` converged; DB head `011`, no migration) — non-US issuers correctly run the LLM councils but the council often returned `insufficient_data` because `company_ir` evidence was **metadata-only**; Phase 29B.2 extracts **bounded, citeable text + high-confidence facts from an issuer's OWN annual-report document** so the council reasons from real **T1 primary** evidence. New backend modules under `apps/api/app/services/sources/`: **`document_fetcher.py`** (`safe_fetch_document` + `DocumentFetchResult` — a bounded, SSRF-safe fetch of ONE allowlisted issuer document; reuses `check_fetch_url` (HTTPS-only, host-allowlist-only, rejects localhost/private/IP-literal/internal hosts + off-domain redirects), enforces `SOURCE_DOCUMENT_EXTRACTION_MAX_BYTES`/`_TIMEOUT_SECONDS`, gates on content-type, sends **no cookies/auth headers**, strips URL secrets, never raises → honest `SourceGap`; **no arbitrary-URL surface** — the URL always comes from the verified-issuer registry or an already-extracted allowlisted link); **`document_text_extractor.py`** (`extract_document_text` → `DocumentTextExtraction` — first-N-pages PDF via **pypdf** (new pure-Python dependency, **no OCR**), bounded excerpt count + per-excerpt chars; a scanned/empty/encrypted PDF returns **no excerpts + an honest gap**; HTML strips scripts/styles/nav; full document text never passed downstream); **`primary_fact_parser.py`** (`parse_primary_facts` → `list[PrimaryFact]` — conservative regex on ONLY high-confidence primary facts (reporting_currency/fiscal_year/revenue/operating_profit/net_income/free_cash_flow/total_assets/total_debt/cash/employees), each carrying value/unit/currency/scale/period/source_url/excerpt_id/page_number/confidence/parser_warning + `needs_human_review=true`; **refuses ambiguous values**; **no inference of missing facts, no currency conversion, no valuation metrics / price targets / fair value / upside-downside** — weak parsing yields excerpts but no facts). New **`apps/api/app/services/llm/evidence_budget.py`** (`apply_evidence_budget`) deterministically compresses the council `EvidencePack` (de-dup by `(url,title,excerpt-hash)`, rank by content tier then factual-excerpt-over-metadata then order, bound item/total-char/per-item-char counts, re-id survivors `E1..En`, set `omitted_evidence_count`/`omitted_reason`, compress duplicate gaps but **never drop all gaps**) so larger primary-source packs no longer balloon the prompt and trip Azure OpenAI TPM quota; wired via `build_evidence_pack(apply_budget=…)`, applied when `SOURCE_CONNECTOR_ENABLED=true`; new config `LLM_COUNCIL_EVIDENCE_MAX_ITEMS` (`20`) / `_MAX_CHARS` (`24000`) / `_MAX_CHARS_PER_ITEM` (`1200`). **Integration:** `CompanyIrConnector` now accepts an injected `document_extractor` → tiered `EvidenceItem`s `company_ir_annual_report_excerpt`/`company_ir_business_description`/`company_ir_risk_excerpt` (`T1_primary_filing`) + `company_ir_financial_fact` (`T1_primary_filing`, `data_quality` B/C by confidence), prioritised ahead of metadata items before the per-source cap; `collect_company_source_evidence` + `live_fetchers.live_document_extractor` wire fetch→extract→parse; `maybe_run_council` passes the live IR-page + document extractors when `SOURCE_DOCUMENT_EXTRACTION_ENABLED=true` and surfaces a compact, **text-free** `primary_documents` summary (title/domain/tier/excerpt_count/fact_count/requires_translation/warnings) under `source_summary_json.llm_council.primary_documents`. **API:** `POST /api/v1/sources/evidence-preview` gains optional `include_document_text`/`max_items`/`max_excerpts` (**still identity-only — no URL field**) + response `document_extraction_performed`. **UI:** `/admin/sources` evidence-preview gains an "Include annual-report text" checkbox + extracted-text / parsed-fact / document-extraction badges; the report page gains a "Primary Documents (extracted)" card (excerpt/fact counts, domain, T1 tier, translation-pending flag, honest "not the full document / human review required" framing); the T1/T2 checklist logic is **unchanged** (still completes only on real persisted T1/T2 citations). New config flags all default **OFF**/conservative: `SOURCE_DOCUMENT_EXTRACTION_ENABLED` (`false`) + `_MAX_BYTES` (`5000000`) / `_TIMEOUT_SECONDS` (`15`) / `_MAX_PAGES` (`20`) / `_MAX_EXCERPTS` (`8`) / `_MAX_CHARS_PER_EXCERPT` (`1200`) / `_ALLOWED_CONTENT_TYPES`; with `SOURCE_DOCUMENT_EXTRACTION_ENABLED=false` behaviour is **exactly Phase 29B.1** (metadata evidence + honest gaps, no document fetch). New dependency `pypdf>=4.0,<6` (`pyproject.toml` + `requirements.txt`). **Limitations (honest):** **no OCR** (scanned PDFs → honest gap); **no local-language translation yet** (French URDs etc. flagged `requires_translation`, pending Phase 30); **no PDF table extraction yet**; regulator connectors still scaffolded; large real-data packs may still partially fail LLM agents under Azure OpenAI gpt-4.1-mini TPM quota (environmental — the evidence budgeter mitigates but does not eliminate it); auth unchanged; `publication_ready` stays false; `human_review_required` stays true; **parsed facts are unverified until human review**. **No** public publishing, recommendation/rating/BUY-SELL-HOLD-WATCH/price-target/fair-value/upside; no publish route. **No DB migration** (still 011). Backend **2002 passing** (+48 Phase 29B.2), ruff clean, no new mypy errors over baseline; frontend typecheck/lint/build clean, **194 e2e passing** (+2). **Staging validation (2026-07-26) — VALIDATED-WITH-ENVIRONMENTAL-NOTE:** merged `793e0a7`, API + Web `commit_sha` converged at `793e0a7`, no migration (head `011`), AUTH_TEST_MODE absent (routes challenge), logs clean; with `SOURCE_DOCUMENT_EXTRACTION_ENABLED=true` the extraction pipeline is provably active, bounded (5MB/15s), safety-gated and fabrication-free (CFR.SW full analysis = final report, `llm_used`, 8/8 agents, `schema_valid=true`, `safety_valid=true`, `human_review_required=true`, `publication_ready=false`, `primary_documents` present count 0; KER.PA honest gap 0 fabricated; BA.LSE → BAE Systems no Boeing; AAPL/AMAT budget bounded; discovery regression PASS; `/admin/sources` OAuth-gated). **Environmental caveat (carry into 29B.3):** every live verified-issuer annual report reached on staging is a scanned/image PDF (no-OCR by design) or an index-only link, so extraction degrades **honestly** to recorded source-gaps with **zero fabricated facts** — hence `primary_documents` is present-but-empty; the excerpt/fact happy-path is not demonstrable on staging until text-based (non-scanned) primary sources or OCR exist. Final staging flags KEPT: `LLM_COUNCIL_ENABLED`=true · `LLM_DISCOVERY_COUNCIL_ENABLED`=true · `SOURCE_CONNECTOR_ENABLED`=true · `SOURCE_DOCUMENT_EXTRACTION_ENABLED`=true. **Next: 29B.3 (deeper primary-fact integration / SEC full-text), 29B.4 (live EU/UK regulated-disclosure connectors), 29C (macro/commodity/policy), 30 (translation/local-language + PDF table extraction).**

### Previously: Phase 29B.1 Non-US Primary Filing + Company IR Evidence ✅ — improves **evidence quality for non-US issuers** (especially European luxury/watch names — Richemont `CFR.SW`, Swatch `UHR.SW`, LVMH `MC.PA`, Hermès `RMS.PA`, Kering `KER.PA`, Burberry `BRBY.LSE`, Pandora `PNDORA.CO`, Moncler `MONC.MI`) whose reports were `insufficient_data` because the pack held only price/model (T5/T6) items — SEC EDGAR does not cover them and their home-regulator connectors are still scaffolded. New **code-defined verified-issuer source registry** (`app/services/sources/verified_issuer_sources.py`): a maintained *allowlist* (never model-fabricated, no secrets, no tokenized URLs) of each issuer's own investor-relations / annual-reports / newsroom URLs + `allowed_domains`; invariants (HTTPS, host∈allowlist, no token params, unique `(ticker,exchange)`, required identity) are test-enforced. New **bounded, SSRF-safe web fetcher** (`safe_web_fetcher.py`): HTTPS-only, host-allowlist-only, rejects localhost/private/IP-literal/internal hosts and off-allowlist redirects, timeout/max-bytes/max-links capped, never raises, secret-free — and **no arbitrary-URL surface** (URLs only ever come from the code-defined registry or a link already extracted from an allowlisted page, re-checked before every request). The **upgraded `CompanyIrConnector`** exposes `search_company` (IR profile), `fetch_filings` (annual-report discovery) and `fetch_events` (press/newsroom): for a verified issuer it always emits bounded, honestly-labelled **`data_quality=metadata_only`** T1 company-source evidence (`company_ir_profile` / `company_ir_annual_reports_index` / `company_ir_press_release_index`) **with no network call** — so KER/UHR/CFR full-analysis packs now carry citeable company-IR evidence beyond price/model; the read-only evidence-preview endpoint additionally does bounded live extraction of annual-report links (`company_ir_annual_report` → `T1_primary_filing`) and press links. Non-US issuers also get honest `connector_scaffolded` ("company IR annual report used as primary source pending regulator integration") and `translation_required` ("pending Phase 30") gaps — **never a fabricated filing or fundamental**. New config `SOURCE_CONNECTOR_MAX_BYTES` (`1000000`), `SOURCE_CONNECTOR_ALLOWLIST_ONLY` (`true`), `SOURCE_CONNECTOR_MAX_LINKS_PER_PAGE` (`25`). Two report-renderer bug fixes: unavailable-section `note`/`disclaimer` envelopes no longer render `[object Object]` (unwrapped via `noteText`), and the **"Data quality: T1/T2 sources present" checklist item is no longer marked complete when only T5/T6 or metadata-only evidence exists** (keyed off real citation tiers, not just `not is_mock`). `/admin/sources` evidence-preview gains non-US quick-fill examples + URL-domain / metadata-only / translation-pending badges (no URL field, no settings editing). Discovery packs unchanged (no per-candidate network fan-out). **No** broad web search, arbitrary-URL fetch, translation agents (30), macro/commodity (29C), public publishing, recommendation/rating/BUY-SELL-HOLD-WATCH/price-target/fair-value/upside; **no** auth change, `human_review_required=true` / `publication_ready=false` unchanged; no publish route. **No DB migration** (still 011). Backend **1954 passing** (+32 Phase 29B.1), ruff clean, mypy clean on touched files (pre-existing ~71-error baseline unchanged); frontend typecheck/lint/build clean, +3 Playwright. **Next: 29B.x (SEC full-text + live regulator fetch), 29C (macro/commodity/policy), 29D (event-trigger/patents/local press), 30 (translation/local-language agents + PDF text extraction).**

### Previously: Phase 29B Filing & Regulator Connector Batch 1 ✅ — the first real filing/disclosure connectors are wired into the Phase 29A framework, prioritising **evidence quality over provider count**. `sec_edgar` and `company_ir` become **live-evidence** connectors: `SecEdgarConnector.fetch_filings` maps already-fetched SEC filing metadata into tiered `EvidenceItem`s (transport `T2_regulator_or_gov` / content `T1_primary_filing`, both preserved), is exchange-aware (non-US → honest `source_not_eligible` gap via Phase 27.1A `is_sec_eligible`, never a wrong-CIK lookup) and attaches a `primary_filing_unavailable` gap because full filing text is not fetched yet; `CompanyIrConnector.fetch_events` wraps issuer press releases as `T1_primary_company_source` evidence (`company_ir_press_release`, URL secrets stripped, bounded, media URLs never cited). Six regulator connectors — **SEDAR+ (CA), ASX (AU), UK FCA NSM (GB), Euronext, Deutsche Börse, Nordic** — become **`scaffolded`** (a new lifecycle status): a real `ScaffoldConnector` class returns honest `connector_scaffolded` `SourceGap`s and **never a fabricated filing/JORC/RNS**. A `collect_company_source_evidence` service runs the connectors over **already-fetched deterministic data** (no report-time network calls) and — when `SOURCE_CONNECTOR_ENABLED=true` (OFF by default) — injects bounded, tiered connector evidence + honest gaps into the single-company evidence pack + council; discovery packs get run-level source-framework gaps only. New read-only **admin/internal** endpoint `POST /api/v1/sources/evidence-preview` (identity-only request, **no URL field**, offline unless the flag is set, then bounded live fetch to SEC EDGAR + curated issuer feeds only). New config `SOURCE_CONNECTOR_ENABLED`/`SOURCE_CONNECTOR_MAX_ITEMS_PER_SOURCE`/`SOURCE_CONNECTOR_TIMEOUT_SECONDS` (conservative defaults). This phase can close with only SEC + company_ir producing real evidence and non-US connectors returning honest gaps. **No** macro/commodity/policy (29C), event-trigger (29D), or translation (30) connectors; **no** auth change, **no** public publishing, **no** recommendation/rating/BUY-SELL-HOLD-WATCH/price-target/fair-value/upside; `human_review_required=true` and `publication_ready=false` unchanged; no publish route. **No DB migration** (still 011). Backend **1910 passing** (+26 Phase 29B), ruff clean, mypy clean on touched files (pre-existing ~71-error baseline unchanged). **Next connector phases: 29B.x (live fetch for the scaffolded regulator connectors + SEC full-text), 29C (macro/commodity/policy), 29D (event-trigger/patents/local press), 30 (translation/local-language agents).**
