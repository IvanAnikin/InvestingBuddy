...
## Current State: Phase 30B Local-Language Evidence Sources 🔜 NEXT — not started (scope + branch `feature/phase-30b-local-language-sources` to follow). With **Phase 30A closed + staging-validated** (the language-detection + translation **foundation** is on `main` at `fa3632a`, OFF by default), Phase 30B adds the **local-language evidence SOURCES** the 30A layer was built to translate. Add local-language business-press / regulated evidence through **allowlisted connectors** (the planned `local_language_business_press` registry row + the `requires_translation`-emitting 29B.4 regulator connectors are the seam) so that non-English excerpts are actually produced, then **consume the 30A translation layer** (bounded per-excerpt, machine-assisted / NOT-official, human-review-required, original text + source URL preserved) and **integrate the translated excerpts into the evidence budget**. Keep **source gaps honest** (honest `data_not_sourced` / `translation_required` gaps, never fabricated headlines / figures / filings); **allowlisted only — NO broad / arbitrary web search** unless explicitly reviewed + bounded. Same discipline: evidence-first, citation-bound, source-registry + `EvidenceItem` tiers + explicit `SourceGap`s, OFF-by-default flags, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated. **After 30B: Phase 31 (source-aware research memo)**, then the wider Phase 30 PDF table extraction / OCR work.
