...
## Current State: Phase 29D.2 Patent Event-Trigger Reference Connectors 🔜 NEXT — **not started** (the **second** Phase 29D subphase; the Phase 29D umbrella remains 🟡 IN PROGRESS after 29D.1 closed). Planned scope: promote the PLANNED registry rows `google_patents` / `uspto` / `epo_espacenet` into **reference-only** patent event-trigger connectors following the SAME 29D.1 event pattern — per relevant theme, ONE bounded **SOURCE REFERENCE** (`source_type` `government_data` or a patent-appropriate type — decide at scoping; fixed official public URL, no API key, **NO specific patent number / claim / applicant / grant date, and NO legal / infringement / validity conclusion**) plus an honest `data_not_sourced` gap; **reference-only, network-free, OFF by default** — reuse the existing `SOURCE_EVENT_ENABLED` flag or add a dedicated `SOURCE_PATENT_ENABLED` flag (decide at scoping). A patent reference is theme-scoped **WEAK CONTEXT** — never company-specific, never a catalyst / materiality / trade signal, never a candidate, never a recommendation, and **never a legal / infringement / patent-validity conclusion**; dark-by-default byte-identical when off; `schema_valid`/`safety_valid` true, `publication_ready` false, `human_review_required` true; `/admin/*` OAuth-gated. **Still upcoming in the Phase 29D umbrella after 29D.2:** **29D.3 permits / regulatory-event metadata**. **Immediate next action:** scope Phase 29D.2 (patent event connectors) and create branch `feature/phase-29d2-patent-event-connectors` (STOP at the merge gate — do not merge / deploy / mark closed until human approval + staging validation is on file). After 29D → Phase 30 (translation / local-language + PDF table extraction / OCR).
