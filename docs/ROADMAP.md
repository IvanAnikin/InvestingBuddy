...
## Current State: Phase 30A Language Detection + Translation Foundation 🔜 NEXT — NOT STARTED (a NEW capability area; no branch yet — next branch `feature/phase-30a-language-translation-foundation`; backend-first, OFF by default). With **all of Phase 29 now COMPLETE** — 29A source-registry + connector framework · 29B filings/regulator + primary docs/facts (29B / 29B.1 / 29B.2 / 29B.3) · 29B.4 EU/UK regulated disclosures (4A/4B/4C) · 29C macro/commodity/policy reference connectors (29C.1/29C.2/29C.3) · 29D event-trigger connectors (29D.1/29D.2/29D.3) — the evidence layer now carries **non-English primary sources**: several 29B.4 regulated-disclosure connectors + event connectors already emit `requires_translation` markers (French / German / Italian / Danish excerpts). Phase 30A lays the **language + translation foundation**: per-evidence-item / per-excerpt **language detection**, **translation-status fields** on `EvidenceItem`, and a **translation-provider abstraction** — all **OFF by default**. Non-English excerpts are flagged `requires_translation`; the **original text + source URL are always preserved** (translation is additive, never destructive); **whole-document translation is out of scope** (bounded excerpt-level only); translated evidence carries a **human-review warning** (never presented as an official translation). Same discipline: evidence-first, citation-bound, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated. **Not started — to be scoped next** (`ib-product-architect` → branch `feature/phase-30a-language-translation-foundation`), then the wider Phase 30 translation / local-language + PDF table extraction / OCR work.
