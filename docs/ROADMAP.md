...
## Current State: Phase 29B.4B Euronext Regulated-Disclosure Connector 🟡 IN PROGRESS — PR-open / pre-staging (branch `feature/phase-29b4b-euronext-disclosures` @ `cd6b02a`; **not yet merged / deployed / staging-validated**). The current subphase of the Phase 29B.4 umbrella (EU/UK regulated-disclosure connectors). Applies the **promote-scaffold→connector pattern proven in 29B.4A** to Euronext: promotes the `euronext_regulated_info` scaffold to a dedicated `EuronextRegulatedConnector` (`apps/api/app/services/sources/connectors/euronext_regulated_info.py`) for verified Euronext issuers — Euronext Paris FR (`MC.PA` LVMH, `RMS.PA` Hermès, `KER.PA` Kering) and Euronext Amsterdam NL (`ASML.AS` ASML), resolved via `verified_issuer_sources`, **never a fabricated filing/headline/date/notice number** — emitting ONE bounded **T2 regulator-transport SOURCE REFERENCE** to the Euronext regulated-information venue + country regulator (AMF France / AFM Netherlands) at the fixed public URL `https://www.euronext.com/en/regulated-information` **plus an honest `primary_filing_unavailable` content gap**, **network-free at report time**. Extends `regulator_connector_for()` (Euronext Paris PA/FR + Amsterdam AS/NL → `euronext_regulated_info`) and tightens `_relevant_scaffold_ids` (UK still `uk_fca_nsm`; DE unchanged); `registry.py` moves `euronext_regulated_info` out of the scaffold table (registry now **8 enabled / 4 scaffolded**: SEDAR+/ASX/Deutsche Börse/Nordic). French-jurisdiction issuers (MC/RMS/KER) are **flagged `requires_translation`** (`GapType.translation_required`; pending Phase 30 — no local-language ingestion this subphase, **not** a claim of official translation). Unresolvable / non-Euronext issuers → honest `source_not_eligible` gap. **Deferral carried forward (from 29B.4A):** the live regulator-venue CONTENT fetch stays deferred (honest T2 venue reference + explicit content gap is the chosen posture); live content fetch remains a future Phase 29B.4 follow-up. Verified GREEN: backend **2058 pass / 12 skip / 0 fail**, ruff clean, mypy `71` baseline (no new), security scan PASS. Safety posture unchanged: evidence-first, citation-bound, no recommendation/rating/BUY-SELL-HOLD-WATCH/price-target/fair-value/upside; `human_review_required=true` / `publication_ready=false`; `/admin/*` OAuth-gated; human approval before publication. **Next: run ib-pr-review-agent then `gh pr create` for 29B.4B and STOP at the merge gate; then 4C (Swiss / Nordic / Germany — `UHR.SW` Swatch / `CFR.SW` Richemont / `PNDORA.CO` Pandora / `SAP.DE` SAP), 29C (macro/commodity/policy), 30 (translation/local-language + PDF table extraction / OCR).**
