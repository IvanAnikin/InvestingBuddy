...
## Current State: Phase 29D Event-Trigger Connectors 🔜 NEXT — not started (the **Phase 29C umbrella is now COMPLETE**: 29C.1 macro baseline `a8ac580` + 29C.2 commodity/energy `80c8454` + 29C.3 policy/government `ad6dde5`, all merged + deployed + staging-validated). Phase 29D adds **timely, sourced event-trigger evidence** on top of the same source-registry / `EvidenceItem`-tier / explicit-`SourceGap` framework, and will **likely split into three metadata-first subphases**: **29D.1 procurement / tenders** (EU TED / government contract awards — promote the `eu_ted` + `usaspending` PLANNED registry rows), **29D.2 patents** (promote the `google_patents` / `uspto` / `epo_espacenet` PLANNED rows), and **29D.3 permits / regulatory-event metadata**. Same Phase 29 discipline, made explicit for events: **metadata-first, honest gaps, no fake awards / contracts / tenders / patents, no broad or arbitrary web crawling**; network-safe / allowlisted; OFF-by-default flags; an event is an **internal research-priority evidence signal only — never a catalyst-as-fact, never a recommendation / rating / valuation / price-target / BUY-SELL-HOLD-WATCH / fair-value / upside**; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated. **Immediate next action:** scope **Phase 29D.1 (procurement / tenders connectors)** and create branch `feature/phase-29d1-procurement-tender-connectors`. After 29D → Phase 30 (translation / local-language + PDF table extraction / OCR).
