...
## Current State: Phase 29C Macro / Commodity / Policy Evidence Connectors 🔜 NEXT — NOT STARTED (no branch, no PR, no code yet — do not mark in-progress or complete). With the **Phase 29B.4 umbrella now COMPLETE** (4A UK FCA NSM `5138725`, 4B Euronext `1d97612`, 4C Swiss / Nordic / Germany `de126ee` — all merged + deployed + staging-validated; the source registry stands at **11 enabled / 2 scaffolded**, only SEDAR+ and ASX regulator connectors remain scaffolded), the next phase widens evidence beyond issuer/regulator disclosure into **macro, commodity/energy and policy/government** signals. Planned to **split into subphases**: **29C.1 macro baseline** (e.g. FRED / IMF / Eurostat / World Bank), **29C.2 commodity + energy** (e.g. USGS / IEA / EIA / IRENA / ENTSO-E / World Bank Pink Sheet), **29C.3 policy + government** (e.g. USTR-TARIC / USAspending / EU TED / UN Comtrade). Same discipline as Phase 29B: **evidence-first, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside**; **prefer official / government sources**; wire through the existing **source registry + `EvidenceItem` tiers + explicit `SourceGap`s** (honest coverage gaps, never fabricated data); **no broad / arbitrary web search**; network-safe (allowlisted hosts, SSRF-safe, bounded); all new connectors **OFF-by-default** behind flags; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated; human approval before publication. **Next exact step: scope Phase 29C.1 (macro baseline connectors) and create branch `feature/phase-29c1-macro-connectors`.**
