...
## Current State: Phase 30B Local-Language Evidence Sources 🟡 IN PROGRESS (PR-open, pre-staging) — branch `feature/phase-30b-local-language-sources` @ `b72bcdf`, **PR about to open; NOT merged/deployed/validated**. With **Phase 30A closed + staging-validated** (the language-detection + translation **foundation** is on `main` at `fa3632a`, OFF by default), Phase 30B adds the **local-language evidence SOURCES** the 30A layer was built to translate. A new **`LocalLanguagePressConnector`** (`sources/connectors/local_language_press.py`) + a `LOCAL_LANGUAGE_PRESS_SOURCES` allowlist of reputable local-language business-press venues (**Les Échos** FR / **Handelsblatt** DE / **Milano Finanza** IT / **Børsen** DK — fixed public HTTPS landing pages, no API key) emits, for a **verified non-US FR/DE/IT/DA issuer**, ONE bounded **T4_quality_media `news_article` SOURCE REFERENCE** (provider `news`) carrying a **GENUINE short local-language descriptive excerpt** (what the venue covers + that the full article content is NOT fetched — **never a fabricated article / headline / quote / figure / date**), marked `requires_translation` + `original_language`, `low` confidence / `metadata_only`, with an honest `translation_required` + content-not-fetched gap + `needs_human_review` (it deliberately **lowers** source quality — a WEAK research-priority signal, never a recommendation / catalyst / materiality / valuation). `company_evidence.py` gains a dedicated `LOCAL_LANGUAGE_REFERENCE_IDS` collection block that runs the connector for verified non-US FR/DE/IT/DA issuers (regulator mapping untouched); `registry.py` promotes `local_language_business_press` PLANNED→**enabled** (`news` / `T4_quality_media`) → **35 enabled / 2 scaffolded / 1 planned / 38 total** (only `openbb` remains planned). It **consumes the 30A translation layer** (with `SOURCE_TRANSLATION_ENABLED` on, the non-English excerpt gets a bounded per-excerpt, machine-assisted / NOT-official, human-review-required translation with the original text + source URL preserved) — **NO new flag** (reuses `SOURCE_CONNECTOR_ENABLED` for collection + `SOURCE_TRANSLATION_ENABLED` for the 30A layer). Verified GREEN (pre-staging): backend **2278 pass / 12 skip / 0 fail**, ruff clean, mypy `71` baseline (no new), security PASS; 15 files (incl. tests). Keep **source gaps honest** (honest `data_not_sourced` / `translation_required` gaps, never fabricated headlines / figures / filings); **allowlisted only — NO broad / arbitrary web search** unless explicitly reviewed + bounded. Same discipline: evidence-first, citation-bound, source-registry + `EvidenceItem` tiers + explicit `SourceGap`s, OFF-by-default flags, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated. **On merge + staging validation, 30B completes the Phase 30 umbrella (30A foundation + 30B local-language sources); next is Phase 31 (source-aware internal research memo builder)**, then the wider Phase 30 PDF table extraction / OCR work.
