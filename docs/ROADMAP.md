...
## Current State: Phase 30A Language Detection + Translation Foundation 🟡 PR OPEN / IN REVIEW — NOT merged/deployed/validated (branch `feature/phase-30a-language-translation-foundation` @ `82150f3`; backend-only, no web change; DB head `011`, **no migration, no new host/endpoint/secret**; four new OFF-by-default flags `SOURCE_TRANSLATION_ENABLED` / `SOURCE_TRANSLATION_MAX_CHARS`(400) / `SOURCE_TRANSLATION_MAX_EXCERPTS`(3) / `TRANSLATION_PROVIDER`(`fake`)). With **all of Phase 29 now COMPLETE** — 29A source-registry + connector framework · 29B filings/regulator + primary docs/facts (29B / 29B.1 / 29B.2 / 29B.3) · 29B.4 EU/UK regulated disclosures (4A/4B/4C) · 29C macro/commodity/policy reference connectors (29C.1/29C.2/29C.3) · 29D event-trigger connectors (29D.1/29D.2/29D.3) — the evidence layer already carries **non-English primary sources** (several 29B.4 regulated-disclosure + event connectors emit `requires_translation` markers for French / German / Italian / Danish excerpts). Phase 30A lays the **language + translation foundation**, **OFF by default**, with the LLM-backed provider deliberately **not** the default. A new `app/services/sources/language.py` exposes `detect_language(text, hint)` for **en/fr/de/it/da** (generalizes the document-extractor stopword heuristic, adds Danish/Nordic) + `LANGUAGE_NAMES`/`language_name()`; `document_text_extractor._detect_language` now delegates (behaviour preserved). A new `app/services/sources/translation.py` defines `TranslationResult` + a `TranslationProvider` ABC with **`FakeTranslationProvider`** (the DEFAULT — an honest, clearly-marked placeholder that NEVER fabricates fluent English) and **`LLMTranslationProvider`** (composes the existing `get_llm_client()` — no new host/model/secret; bounded input AND output; logs ONLY counts + language codes, **never** the prompt/original/translated text), selected by `get_translation_provider(cfg, *, client)`. Every result carries a `MACHINE_TRANSLATION_WARNING` ("machine-assisted, NOT an official translation; human review required") + `needs_human_review=True`, is **bounded per-excerpt (never whole-document)**, and always preserves the **original text + source URL**. The single-company council (`_collect_translated_excerpts`, gated by `SOURCE_TRANSLATION_ENABLED`) surfaces bounded translations as `CouncilResult.translated_excerpts` metadata (empty `[]` when off — mirroring the macro/event precedent), and the final report gains an optional `translated_evidence` block (safety-scanned before validation). **Interpretation call (documented):** translated excerpts are council metadata + a report block (each retaining the original source URL as the citation of record), **not** injected into the evidence pack; original evidence is untouched. Same discipline: evidence-first, citation-bound, no recommendations / ratings / valuations / price-targets / BUY-SELL-HOLD-WATCH / fair-value / upside; `human_review_required=true` / `publication_ready=false` unchanged; `/admin/*` OAuth-gated. Verified GREEN (pre-staging): backend **2264 pass / 12 skip / 0 fail** (+`test_phase30a_translation.py`), ruff clean, mypy `71` baseline (no new), security scan PASS; 9 files (incl. tests). **Not yet validated — staging validation pending; the phase stays 🟡 IN REVIEW until merge SHA + deployed SHA + staging validation are on file.** **Next: run `ib-pr-review-agent`, open the PR, and STOP at the merge gate** (no merge/deploy/close until human approval + staging validation). **After 30A validates: Phase 30B (local-language evidence sources — consume the 30A translation layer) is next, then Phase 31 (source-aware research memo)**, then the wider Phase 30 PDF table extraction / OCR work.
