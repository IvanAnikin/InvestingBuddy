...
## Current State: Phase 31 Source-Aware Internal Research Memo Builder 🔜 NEXT — the FINAL phase (not started). **With the Phase 30 umbrella now ✅ COMPLETE** (30A translation foundation `fa3632a` + 30B local-language evidence sources `e1d2d8d`, both merged + deployed + staging-validated), the evidence layer is feature-complete: source registry + connector framework (29A), filings / regulator + primary docs / facts (29B / 29B.1–3, 29B.4 EU/UK regulated disclosures), macro / commodity / policy reference connectors (29C.1–3), event-trigger connectors (29D.1–3), and translation / local-language sources (30A/30B). **Phase 31 is the last phase** — it assembles a readable **internal admin research memo** from data the system already holds (evidence packs, explicit source gaps, LLM-council outputs incl. disagreement, primary facts, red-team critiques), most likely as a new report section / renderer over the existing `final_report` / council / evidence data (no new evidence sources). The memo is **citation-bound** (each claim carries its source tier), **degrades honestly when evidence is thin** (says what is missing rather than filling gaps), and is **strictly internal**: **no public publishing, no recommendation labels, no valuation, no price targets, no BUY/SELL/HOLD/WATCH / fair-value / upside**. Memo structure (per the campaign spec): **header** (internal-only — NOT investment advice) · **company identity** · **why surfaced** · **what's sourced** · **what's missing** · **primary evidence** · **catalyst / event evidence** · **financial facts** · **business / risk** · **council disagreement / red-team** · **research next steps** · **human-review checklist** · **source appendix** · **disallowed-outputs notice**. Same discipline: evidence-first, source-registry + `EvidenceItem` tiers + explicit `SourceGap`s (never fabricate a claim/figure/citation), OFF-by-default where a flag is warranted, `human_review_required=true` / `publication_ready=false` unchanged, `/admin/*` OAuth-gated, human approval before any publication. **Next command:** scope Phase 31 (source-aware internal research memo builder) and create branch `feature/phase-31-research-memo-builder`.
