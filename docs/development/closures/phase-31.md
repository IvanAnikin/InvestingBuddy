...
2. **Depends on upstream evidence richness** — a thin source report yields a
   ...
   - *Follow-up (hotfix — PR #70 `8cc21a6`, now MERGED + deployed + STAGING-VALIDATED 2026-07-30, VALIDATED — WITH ENVIRONMENTAL NOTES):* addresses the report/memo **surfacing** of verified METADATA-ONLY primary-source references (issuer IR / annual-report / press index) so they are no longer shown as `0` primary documents / `0/0` source appendix when no text/facts are extracted. On staging, a new CFR/SW full report `1e18aa4d-d6f4-4865-ae40-93984c10032c` now surfaces **6 primary-source references** (5 richemont.com + 1 six-group.com T2 venue) while honestly reporting `0` extracted documents / `0` primary facts (`extracted_document_text_available=false` / `primary_facts_available=false`), `schema_valid`/`safety_valid` true, `publication_ready` false, `research_complete` false. It still does **not** add OCR / extraction — it re-presents references the evidence layer already holds. See PHASE_LEDGER row `31-hotfix` and closure `docs/development/closures/phase-31-hotfix.md`.
