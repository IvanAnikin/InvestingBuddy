...
| `BACKEND_API_BASE_URL` / `BACKEND_BASIC_AUTH` | Phase 17+ | Server-only proxy target + Basic Auth (never `NEXT_PUBLIC_`); now server-to-server defense behind the admin session |
| `AUTH_SECRET` | Phase 23+ | Signs the admin session cookie (`openssl rand -base64 32`) — required; Key Vault ref in staging |
| `ADMIN_ALLOWED_EMAILS` | Phase 23+ | Comma-separated admin allowlist; empty = nobody authorized |
| `AUTH_GITHUB_ID` / `AUTH_GITHUB_SECRET` | Phase 23+ | GitHub OAuth App credentials for admin sign-in |
| `AUTH_TRUST_HOST` / `AUTH_URL` | Phase 23+ | Trust proxy headers / explicit external origin for OAuth redirect URIs |
| `AUTH_TEST_MODE` | Phase 23+ | `true` enables deterministic dev/CI sign-in — **local/CI only**, never staging/prod |
...
| Phase 24.1.2 | Done | Press-Release Canonical Link Fix: company press-release `source_url` is now always the canonical article page, never an image/media URL. Root cause = Apple's Atom feed has two `<link>` per entry (article + `rel="enclosure"` image `…jpg.og.jpg`) and the parser let the last one win. Adds `extract_canonical_feed_link()` + `is_media_url()` (rejects image/video extensions, `/images/`·`/media/`·`/thumbnail/`·`/tile/`, `.og.jpg`, `media:content`/`media:thumbnail`, enclosure images, description `<img>`); keeps the image as `NewsItem.media_url`; `CatalystEvent` gains `media_url` + `source_url_quality` (`canonical_article`/`rejected_media_only`/`missing`); dedup by canonical URL; T1 only for a verified company-owned article/feed; 21 backend + 1 Playwright tests. No recommendations, price targets, fair values, or upside/downside; human review required; `safety_valid` stays true |
| Phase 25 | Done | Real Market Candidate Discovery (internal-only): moves from manual single-ticker analysis to a bounded market scan producing an internal research-candidate queue. `discovery_runs` + `discovery_candidates` (migration 010); deterministic `discovery_scoring_service` (`0.30·momentum + 0.25·catalyst + 0.20·fundamentals + 0.15·source_quality + 0.10·completeness − risk_penalty`, 0–100, internal prioritization only; grades high/medium/low internal interest + data_insufficient); `discovery_signal_extractor` reuses the tested `company_analysis` workflow per ticker (injectable → offline CI); `market_discovery_service` orchestrator (universe validated against `DISCOVERY_MAX_UNIVERSE_SIZE`, empty/oversized rejected 422, non-blocking per-ticker failures, forbidden-term safety scan); 6 admin-only `/api/v1/market-discovery/*` endpoints; `/admin/discovery` dark-glass UI (start-run form + universe-size preview + runs table + ranked candidate queue + inline detail + "Run Full Analysis"); config `DISCOVERY_*` (seed `AAPL,MSFT,NVDA,AMZN,GOOGL,META,TSLA`); 51 backend + 15 Playwright tests. No BUY/SELL/HOLD/WATCH, no price targets/fair value/upside/downside, no recommendations, no public publishing; every candidate `human_review_required=true` / `is_public=false` |
| Phase 25.1 | Done | Async Discovery Run Execution (UX/ops hardening; no migration): `POST /api/v1/market-discovery/runs` returns a `pending` run immediately and processes the universe in the background — FastAPI `BackgroundTasks` in a **fresh** DB session (`create_pending_run` + `process_run` + `process_discovery_run_by_id`/`process_discovery_run_task`), committing progress after every ticker; run schema gains computed `progress_pct` + `is_async`/`message`; `/admin/discovery` polls `GET /runs/{run_id}` (3s) with a live status badge + progress bar + processed/candidate/error counts, an empty-queue "candidates will appear as tickers finish" placeholder, and a friendly timeout message if the POST ever fails. Removes the multi-ticker `504` under the single B1 worker; guards against reprocessing a terminal run / double-running a fresh `running` run (30-min stale restart); `BackgroundTasks` are process-local (durable queue deferred); oversized/empty universe still rejected 422 before scheduling; 27 backend + 9 Playwright tests. Safety unchanged — no BUY/SELL/HOLD/WATCH, no price targets/fair value/upside, `human_review_required=true` / `is_public=false` |
| Phase 23 | Done | Admin/Auth Hardening: `/admin/*` and `/api/admin/proxy/*` now require an authenticated, allowlisted admin. Dependency-free HMAC-signed httpOnly session cookie (`AUTH_SECRET`), GitHub OAuth sign-in (secret used server-side only, access token discarded after reading the verified email), env allowlist (`ADMIN_ALLOWED_EMAILS`), Next 16 **Proxy** (`src/proxy.ts`) gating pages (→ `/login` / `/unauthorized`) and the proxy API (401/403), independent re-check + advisory `X-IB-Admin-Email`/`X-IB-Admin-Name` audit headers in the proxy route before Basic Auth is attached, `/login` + `/unauthorized` pages, signed-in identity + Sign out in the admin shell, and `AUTH_TEST_MODE` deterministic dev/CI sign-in. Backend Basic Auth retained as server-to-server defense (extracted to `install_staging_basic_auth`; never trusts the identity headers without Basic Auth). No migration; no public publishing; no recommendation output; all internal-only disclaimers preserved. Backend +13 tests (1236 total); frontend +15 auth Playwright specs (auth fixture; existing admin specs sign in) |
